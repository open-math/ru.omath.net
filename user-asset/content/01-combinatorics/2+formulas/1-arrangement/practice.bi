...
~ aRepeat article|a-repeat-formula
~ aUnique article|a-unique-formula
...
@problem
    ...
    answer: |
        ...
        Две белые засечки на дисковом замке в правом верхнем углу могут означать, что для его открытия требуется указать два числа.

@problems
    title: Мастер по семафорам
    level: easy
    shared: |
        На картинках изображены оптические телеграфы.\
        Найдите, сколько уникальных сигналов можно передать с их помощью.
    set:
        -   description: |
                Такие использовались в Норвегии во время наполеоновских войн.
        ...
        -   description: |
                На картинке изображён концепт семафора за авторством англичанина Ричарда Лоуэлла.
                Он состоит из $4$ треугольников, которые могут вращаться с шагом в $45$ градусов.
...
@problem
    ...
    level: easy
    description: |
        Пусть при том же условии из задачи [Большое путешествие](big-journey) два путешественника выезжают из Николаева по разным шоссе.
...
@problem
    ...
    solution: |
        ...
        Размещение по [определению](article|a-repeat) -- **упорядоченная** комбинация, то есть порядок расположения элементов в ней имеет значение.
...
@problem
    ...
    answer: |
        $$ \bar{A}_{2}^{32} = 2^{32} = 4 \ 294 \ 967 \ 296 $$
    note: |
        Сейчас государств с более чем $4$ миллиардами жителей на Земле не существует.
        Какую страну ни возьми, всего многообразия наборов зубов не найдешь!
        Вообще, почти наверняка, что этого многообразия не найдется и в целом по всей планете, хотя теоретически это и возможно.
...
@problem
    ...
    hint: |
        Подумайте, что происходит с последним вакантным местом при размещении без повторений.
        Делаем ли мы какой-то выбор для него?
    solution:
        ...
        "Смысл равенства": |
            ...
            На предпоследнее вакантное место у нас остается в запасе всего $2$ элемента.
...
@problem
    ...
    hint: |
        Проще найти не количество совпадений в решениях судей относительно первого места, а наоборот, варианты, когда они все выбрали разных гимнастов на первое место.
    solution: |
...
@problem
    ...
    answer: |
        ...
        Отец отдает все апельсины. Ограничений на количество апельсинов у сыновей нет:
        ...
        Отец может отдать не все апельсины. Ограничений на количество апельсинов у сыновей нет:
...
@problem
    ...
    solution: |
        ...
        Находим количество этих размещений по [формуле](~aRepeat):
...
@problem
    ...
    solution: |
        ...
        Найдем все такие числа [прямым перебором](page|article|~/brute-force).
