...
~ arrangements article|~/arrangement|a-unique-factorial-formula
~ permutationsRepeat article|~/permutation|permutation-repeat-formula
~ combinations article|~/combination|combination-formula
~ combinationsRepeat article|~/combination|combination-repeat-formula
...
@important
    ...
    main: |
        ...
        Прежде чем решать эти задачи, убедитесь, что вы по-отдельности закрепили навыки работы с правилами [суммы](page|practice|~/rules/sum) и [умножения](page|practice|~/rules/product), [размещениями](page|practice|~/arrangement), [перестановками](page|practice|~/permutation) и [сочетаниями](page|practice|~/combination).
...
@problem
    ...
    hint: |
        Последовательно найдите способы рассадить желающих сидеть по движению, потом против движения, потом безразличных.
    solution: |
        ...
        Троих безразличных по трём оставшимся местам в купе можно рассадить $P_3$ способами.
...
@problem
    ...
    solution: |
        Разобьём все семизначные числа на два класса.

        В первом классе числа, которые начинаются на цифру $7$.
        Тогда остальные $6$ вакантных мест в шестизначном числе можно занять $9$ цифрами (все, кроме цифры $7$).
        Сделать это можно $\bar{A}_9^6$ способами.

        Во втором классе числа, которые не начинаются на $7$.
        ...
        Так как выбор первой цифры и положение цифры $7$ не влияют на количество вариантов выбрать остальные цифры, то по правилу умножения получаем всего $6\cdot 8 \cdot \bar{A}_9^5$ чисел во втором классе.
...
@problem
    title: Пятёрка лучших!
    level: medium
    description: |
        Из $12$ девушек и $10$ юношей выбирают команду в составе $5$ человек.
        Сколькими способами можно выбрать эту команду так, чтобы в неё вошло не более $3$ юношей?
    hint: |
        Сложите варианты собрать команду без юношей, с одним юношей и так далее.

        Или вычтите из всех возможных вариантов собрать команду варианты только из юношей или с четырьмя юношами.
    solution:
        Через сумму: |
            ...
            Из трёх юношей и двух девушек: $C_{10}^3 C_{12}^2$.
        ...
        Через разность: |
            ...
            Найдём сначала способы составить команду без каких-либо ограничений, то есть выбрать пятерых человек из $12 + 10 = 22$: $C_{22}^5$.

            Теперь из всех этих вариантов вычтем составы команды только из юношей и с четырьмя юношами:
...
@problem
    ...
    hint: |
        Найдите количество способов выбрать пару абонентов из $n$, потом пару из $n-2$ и так далее.
    ...
    solution: |
        ...
        Например, пара абонентов $a \leftrightarrow b$ может быть выбрана во время **любого** из трёх выборов пар: в $C_n^2$, в $C_{n-2}^2$ или в $C_{n-4}^2$.
...
@problem
    ...
    solution:
        Кофеварка: |
            ...
            Расстановка согласных никак не влияет на количество способов расставить гласные.
...
@problem
    ...
    solution: |
        Представим прямую.
        Одна точка деления делит её на $2$ части.
        $2$ точки деления делят её на $3$ части.
        Значит $n-1$ точек деления делят её на $n$ частей.
...
@problem
    ...
    hint: |
        Выберите сначала $4$ девушки, а потом $4$ парня или адаптируйте решение задачи [Комбинаторный оператор](calls).
    solution:
        Через комбинаторные конфигурации: |
            ...
            Порядок "назначения" уже имеет значение, поэтому сделать это можно $A_{15}^4$ способами.
        ...
        Через правило умножения: |
            ...
            По такой же схеме, вторую пару можно составить $11\cdot 14$ способами, третью $10\cdot 13$ и четвёртую $9\cdot 12$.
            ...
            Как и в задаче [Комбинаторный оператор](calls), при таком составлении пар среди них есть дубликаты.
...
@problem
    ...
    hint: |
        Найдите количество способов выбрать $k$ пальцев, а затем количество способов надеть на них $k$ колец.
    ...
    solution: |
        ...
        Если всё это внимательно рассчитать и сложить, то получится $1 \ 442\ 172$.
        Как видим, у Кати есть огромное количество разных способов "нарядить" свои пальчики...
    answer: |
        $$ \sum\limits_{k=1}^6 C_6^kA_{12}^k = 1 \ 442\ 172 $$

@problem
    title: Чётных и нечётных пополам!
    level: medium
    description: |
        Во скольких шестизначных числах есть $3$ чётные и $3$ нечётные цифры, если допускаются и "шестизначные" числа, начинающиеся с нуля?
        А если не допускаются?
    hints:
        - Сначала найдите количество способов выбрать места для чётных цифр.
        - Ищите количество способов расставить чётные цифры по ранее выбранным местам, а затем расставляйте нечётные цифры.
        - Если ноль в начале числа не допускается, вычтите из всех полученных шестизначных чисел те, которые начинаются с $0$.
    solution: |
        Выбрать $3$ места для чётных цифр в шестизначном числе можно $C_6^3$ способами.
        По выбранным трём местам эти самые чётные цифры можно разместить $\bar{A}_5^3$ способами.
        Вне зависимости от того, как мы распределили чётные цифры, на оставшиеся $3$ места нечётные цифры можно расставить $\bar{A}_5^3$ способами.
        Используем [правило умножения](~productRule):

        $$ C_6^3\bar{A}_5^3\bar{A}_5^3 = 312 \ 500 $$

        Количество чисел, начинающихся с $0$ рассчитывается точно так же, только всего цифр в числе у нас уже $5$, а чётных цифр будет $2$:
        ...
        Количество чисел, начинающихся на $0$ столько же, сколько чисел, начинающихся на $1$, $2$ или любую другую цифру.
...
@problem
    ...
    solution: |
        ...
        Между львами есть $4$ "вакантных" места для тигров, плюс ещё по месту до львов и после.
...
@problem
    ...
    level: medium
    description: |
        На полке находятся $m + n$ различных книг, из которых $m$ в чёрных переплётах, а $n$ в красных.
        Сколько существует перестановок этих книг, при которых книги в чёрных переплётах занимают первые $m$ мест?
        Во скольких случаях все книги в чёрных переплётах стоят рядом (не обязательно первыми)?
    hint: |
        Сначала найдите способы расставить чёрные книги.
        Потом красные.
        Используйте [правило умножения](~productRule).
    solution: |
        Чёрные книги в начале полки можно расположить $P_m$ способами.
        Вне зависимости от того, как мы чёрные книги расставим, красные книги за чёрными можно расположить $P_n$ способами.
        По [правилу произведения](~productRule) получаем всего $P_m P_n = m!n!$ способов.

        Если чёрные книги стоят рядом, но не обязательно первыми, то есть $(n+1)$ способов их "пачкой" разместить где-то на книжной полке: начиная с первых мест (первый способ) и смещая вправо на одну книгу (ещё $n$ способов):

        $$ (n+1) \cdot P_m P_n = m!(n+1)! $$
    answer: |
        Если чёрные книги стоят на первых местах:

        $$ P_mP_n = m!n! $$

        Если чёрные книги не обязательно на первых местах:
...
@problem
    ...
    solution:
        "": |
            ...
            При любом из этих способов оставшиеся три буквы по трём местам в списке можно расположить $P_3$ способами.
            Применяем [правило умножения](~productRule):
        ...
        Через перестановки: |
            Без учёта ограничений задачи буквы по списку можно расположить $P_5 = 5!$ способами.
            В любом способе есть буквы $А$ и $Б$. На каждый нужный нам вариант, где $А$ стоит **до** $Б$ найдётся ровно один вариант, в котором эти буквы стоят **наоборот**, нарушая условие.
        ...
        Через сочетания: |
            ...
            Оставшиеся три буквы по трём местам в списке можно расположить $P_3$ способами:

            $$ C_5^2 P_3 = 10 \cdot 6 = 60 $$

        Через правило суммы: |
            Если $А$ стоит на первом месте, то букву $Б$ можно расположить $4$ способами в списке после $А$ и ещё $P_3$ способов расставить три оставшиеся буквы по трём местам.

            Если $A$ стоит на втором месте, то букву $Б$ можно расположить $3$ способами в списке после $А$ и ещё $P_3$ способов расставить три оставшиеся буквы по трём местам.
...
@problems
    ...
    set:
        -   description: |
            ...
            solution: |
                Найдём при помощи [перестановок с повторениями](~permutationsRepeat) количество всех слов, которые можно составить из этих букв:

                $$ P(4, 1, 1, 1, 1) = \frac{8!}{4! \ 1! \ 1! \ 1! \ 1!} = 1680 $$

                Четыре "склеенные" буквы "е" в восьми вакантных местах можно разместить пятью способами.
                В любом из этих способов оставшиеся четыре согласные можно разместить по четырём местам $P_4 = 4!$ способами.
                Применяем [правило умножения](~productRule):

                $$ 5 \cdot P_4 = 5! = 120 $$

                Вычитаем из всех возможных слов слова с четырьмя "е" подряд и так находим количество слов, в которых "е" не идёт четыре раза подряд:
...
@problem
    ...
    solution: |
        Для начала найдём все варианты выписать буквы вообще без "о".
...
@problem
    ...
    hint: |
        Сначала поставьте три согласных на чётные позиции, а затем расставьте все остальные буквы.
    solution: |
        Сначала разместим $3$ согласных из $5$ по чётным позициям.
...
@problem
    ...
    hint: |
        Повторите [решение](o-letters) задачи про буквы "о".
        Потом сделайте поправку на то, что гласные разные.
    solution: |
        Повторяем [решение](o-letters) задачи про буквы "о".
    ...
    set:
        -   description: |
            ...
            solution: |
                ...
                В месяце $30$ дней, причём какие-нибудь $20$ из них мы трогать не можем, они заняты отдыхом.
            ...
            note: |
                ...
                Но вот перечислить их задача уже нетривиальная.
        ...
        -   description: |
                ...
                В течение $n$ дней надо сделать $k$ инъекций с перерывами на реклассацию после каждой не менее чем на $s$ дней.
    ...
    set:
        -   description: |
            ...
            hints:
                ...
                - "Ещё можете выделить одного рыцаря, назвать его $A$ и рассмотреть два класса: отряды с рыцарем $A$ и отряды без рыцаря $A$."
                - "В любом из этих двух подходов вам понадобится воспользоваться результатами задачи [Потехе время, а работе час](random-schedule)."
            solution:
                Решение "по кругу": |
                    Первого рыцаря можно выбрать $12$-ю способами.

                    Вне зависимости от выбранного первого рыцаря нам нужно выбрать (без разницы в каком порядке) ещё $4$ рыцаря, **НО**:

                    * $1$ рыцаря мы только что выбрали.
                    * Его $2$-е соседей нельзя выбирать, ведь они противники уже выбранного рыцаря.
                    * Между каждыми из $4$ рыцарей обязательно должен быть "пробел", чтобы не взять противников. Всего $3$ "пробела". (см. решение [Потехе время, а работе час](random-schedule))
                ...
                Решение "в ряд": |
                    Возьмём какого-нибудь произвольного рыцаря и назовём его $A$.
                    Будем рассматривать два класса:

                    1. Все способы собрать отряд вместе с рыцарем $A$
                    2. Все способы собрать отряд без рыцаря $A$

                    В классе с рыцарем $A$ мы по сути разорвали круг и выстроили рыцарей в ряд, начиная с $A$:
...
@problem
    ...
    hint: |
        Это задача -- частный случай [задачи](knights) о рыцарях короля Артура, только в этот раз всего рыцарей $n$, а выбираем из них мы $3$.
        Найдите, сколько останется доступных для выбора вершин после выбора любой первой вершины треугольника.
    solution:
        "": |
            Все действия точно такие же, как и при решении [задачи](knights) о рыцарях короля Артура, только в этот раз всего рыцарей $n$, а выбираем из них мы $3$.
...
@problem
    ...
    solution: |
        Какой $k$-угольник ни возьми (выпуклый или невыпуклый) после каждой из $k$ вершин обязательно должен идти пропуск следующей соседней вершины, иначе возникнет общая сторона.

        Таким образом для построения $k$-угольника нам необходимо $k$ вершин самого $k$-угольника и ещё $k$ нетронутых вершин.
...
@problem
    ...
    hint: |
        Выпишите только гласные и найдите количество способов выбрать между и по бокам от них места под согласные.
        Заполните эти места согласными.
    solution: |
        ...
        Нам нужно выбрать из них $3$ места под согласные, причём места могут повторяться, но их порядок значения не имеет.
        Количество способов это сделать можно посчитать по [формуле](~combinationsRepeat) сочетаний с повторениями: $\bar{C}_5^3$.

        Позиции для согласных мы выбрали.
        Вне зависимости от выбранных позиций три согласные по ним мы можем разместить $P_3$ способами.
        Используем правило умножения:

        $$ \bar{C}_5^3 P_3 = 210 $$

        Со словом "параллелизм" алгоритм точно такой же, вот только там некоторые согласные повторяются, поэтому надо использовать [формулу](~permutationsRepeat) перестановок с повторениями:
...
@problem
    ...
    hint: |
        ...
        Затем встраивайте её в итоговое слово.
    solution: |
...
@problem
    ...
    hint: |
        ...
        Для ответа на второй вопрос расставьте согласные и выберите "позиции" для букв "о".
    solution: |
        Всего буквы можно расставить $P(3,1,1,1)$ способами.
        Три согласные можно расставить $P_3$ способами и $4$ способа встроить между ними ($2$) и по бокам (ещё $2$) "склеенную" тройку букв "о".
        Вычитаем из всех способов варианты с тройной "о":

        $$ P(3,1,1,1) - P_3 \cdot 4 = 96 $$

        Второй вопрос ещё проще.
        ...
        Расстановка согласных никак не влияет на способы выбрать позиции для букв "о", поэтому для нахождения всех способов можно использовать [правило умножения](~productRule):
    ...
    set:
        -   description: |
            ...
            hints:
                ...
                - Из общего количества чисел вычтите количество чисел с тремя тройками.
            solution:
                "": |
                    Сначала нам нужно найти, а сколько вообще можно составить из данных цифр пятизначных чисел.

                    Для этого все возможные пятизначные числа разобьём на группы по количеству одинаковых цифр в их записи.
                    Например, группа $(2, 1, 1, 1)$ означает, что в пятизначном числе $2$ одинаковые цифры и $3$ уникальные (как в числе $11234$).
                ...
                Подсчёт чисел в группах: |
                    ...
                    Если уникальной цифрой выбрать $1$, $2$ или $3$, то в каждом из этих трёх вариантов дубликаты [можно выбрать](duplicate-explain) единственным способом.
                    ...
                    @details
                        ...
                        Такая же ситуация, если в качестве уникальной цифры взять $2$ или $1$.
                ...
                Количество чисел с тремя тройками: |
                    Найдём количество чисел с тремя тройками.
                    Пускай у нас уже есть три цифры $3$ в числе.
                    Осталось расположить ещё две цифры.
                    ...
                    Всего $8$ способов заполнить два места и $3$ возможных расположения этих двух мест.
        ...
        -   description: |
                Сколько четырёхзначных чисел можно составить из цифр числа $123 \ 153$?
            answer: $102$
