~ permutations article|permutation-formula
~ permutationsRepeat article|permutation-repeat-formula
...
@problem
    ...
    description: |
        ...
        Способности можно менять местами, получая различные варианты панели.
    ...
    solution: |
        ...
        Найдём количество перестановок по [формуле](~permutations):
...
@problem
    ...
    solution:
        "": |
            ...
            Если друзей шестеро, то добавим к ним ещё один элемент -- пустоту, которую тоже можно "усадить" на место в машине.
        ...
        "Решение \"наоборот\"": |
            ...
            В практикуме по размещениям мы [доказывали](practice|~/arrangement|fate-exists) вот такое интересное равенство:
            ...
            В чём смысл?
            А в том, что распределив $6$ мест по друзьям, последнее седьмое место **единственным** образом достаётся оставшемуся другу.
...
@problem
    ...
    hint: |
        Смотрите решение аналогичной [задачи](article|number-master) из статьи.
    solution: |
        ...
        Количество перестановок во втором вопросе тоже равно $120$, но среди полученных чисел есть четырёхзначные, начинающиеся с $0$.
        Всего их $P_4 = 4! = 24$.
        За подробностями смотрите [пример](article|number-master) из статьи.
...
@problem
    ...
    solution:
        "Решение через перестановки": |
            Логика решения простая: находим количество всех вариантов, потом количество вариантов, в которых команда $A$ заняла первое и последнее места. Вычитаем из всех вариантов лишние. PROFIT!

            Для начала найдём количество всех возможных результатов соревнования.
...
@problem
    ...
    hint: |
        Число чётное, если последняя его цифра чётная и наоборот.
    solution: |
        Число чётное, только если последняя его цифра чётная, и наоборот.
        Среди данных цифр чётных две ($6$ и $4$).
        ...
        Вне зависимости от сделанного выбора, оставшиеся разряды можно заполнить $6$ способами.
        Используем [правило умножения](~productRule):

        $$ 2 \cdot P_3 = 2 \cdot 6 = 12 $$

        Всего $12$ чётных четырёхзначных чисел.
        И $12$ нечётных, так как нечётных цифр тоже $2$.
    answer: Можно составить $12$ чётных и $12$ нечётных четырёхзначных чисел.

@problem
    title: Безопасность Элли
    level: easy
    description: |
        Элли придумывает секретный пароль к своему компьютеру, причём каждый символ она использует только один раз.
    ...
    solution: |
        Раз повторяющихся элементов нет, то каждый пароль длины $k$ по сути является перестановкой из $k$ символов.
        Осталось лишь подобрать такое $k$, чтобы количество этих перестановок было не меньше $250 \ 000$.
        ...
        Жаль только, она не знает, что эти $362 \ 880$ с помощью [брутфорса](article|~/brute-force|brute-force) можно перебрать за пару секунд...
    answer: $9$
...
@problem
    ...
    description: |
        ...
        А сколько способов, если финиш они пересекают в любом порядке (но все ещё друг за другом)?
    hint: |
        Найдите, сколькими способами группу из трёх подруг можно расположить в таблице результатов марафона.
        Потом найдите, сколькими способами при каждом таком расположении можно разместить остальных участников марафона.
    solution: |
        У нас есть группа из трёх человек, которую нужно поместить в таблицу результатов.
        ...
        Вне зависимости от того, на какое из $14$-ти мест в таблице мы разместили группу из трёх подруг, оставшиеся $13$ мест могут занять любые остальные участники, причём сделать они это могут $P_{13} = 13!$ способами.

        По [правилу умножения](~productRule) находим все возможные способы, которыми группа из трёх подруг может завершить марафон:

        $$ 14 \cdot P_{13} = 14 \cdot 13! = 14! $$

        К такому же результату можно прийти, представив, что группа из трёх девочек -- это один трёхголовый монстр.
        ...
        Если же финиш подруги пересекают друг за другом, но в любом порядке, то нужно сначала найти все варианты составить из трёх подруг группу.
    ...
    answer: |
        ...
        Финишировать как группа, без определённого порядка, подруги могут $6 \cdot 14!$ способами.
...
@problem
    ...
    description: |
        ...
        Каждый день в течение $9$ дней подряд она выдаёт сыну по одному фрукту.
...
@problem
    ...
    hint: |
        Используйте [формулу](~permutationsRepeat) перестановок с повторениями.
    solution: |
        Начнём со слова "ингредиент".
        В нём $10$ букв, из которых надо составлять анаграммы.
...
@problem
    ...
    description: |
        ...
        *"На полке надо расставить две одинаковые книги и одну фигурку. Сколькими способами это можно сделать?"*
        ...
        Он считает, что каждый вариант расставить вещи на полку можно рассматривать как размещение с повторениями из $2$ элементов (книга и фигурка) по $3$ вакантным местам и находит количество способов это сделать по [формуле](article|~/arrangement|a-repeat-formula):
    ...
    hint: |
        Используйте важное [отличие](article|different-repeat).
    solution: |
        Ошибка Миши заключается в том, что размещения с повторениями допускают неограниченное использование элементов.
        Например, в его решении допустим вариант с размещением трёх одинаковых книг, хотя доступно их всего $2$, а также вариант с размещением $3$ фигурок, хотя она всего одна.

        Поэтому, в этой задаче надо использовать [формулу](~permutationsRepeat) перестановок с повторениями, которые [отличаются](article|different-repeat) от размещений с повторениями тем, что дубликаты используются ровно столько раз, сколько их есть в наличии.
...
@problem
    ...
    description: |
        ...
        *"Haec immatura a me iam frustra leguntur oy"*
        ...
        Это предложение можно перевести как "*Эти незрелые вещи пытаюсь объединить я пока что безуспешно*".
        Сколько различных анаграмм можно из неё составить?
    hint: |
    ...
    solution: |
        Слева-направо рассматриваем каждую букву, считаем, сколько раз она повторяется в тексте и вычеркиваем её:
        ...
        Буква "h" встречается один раз, можно её не учитывать.
        Буква "a" встретилась $6$ раз.
        Вычеркиваем и подсчитываем повторяющиеся буквы по такому же алгоритму и дальше, после чего используем [формулу](~permutationsRepeat) перестановок с повторениями:
...
@problem
    ...
    hint: |
        Внесите небольшое изменение в процесс решения задачи [Бесконтактный бой](chess-fight).
    solution:
        "Решение через перестановки": |
            В решении задачи [Бесконтактный бой](chess-fight) мы сначала расставили ладьи в ряд.
...
@problems
    ...
    level: medium
    shared: Во всех примерах найдите $n$.
    set:
        -   description: |
                $$ \frac{P_{n+5}}{P_{n-k}} = 240 A_{n+3}^{k+3} $$
            hint: |
                Используйте [формулу](~permutations) числа перестановок, факториальную [формулу](article|~/arrangement|a-unique-factorial-formula) числа размещений без повторений и рекуррентную [формулу](article|~/factorial|factorial-recurrent) факториала.
            solution: |
                Используем [формулу](~permutations) для числа перестановок и [факториальную](article|~/arrangement|a-unique-factorial-formula) формулу числа размещений без повторений:
                ...
                Для числителя несколько раз подряд используем рекуррентную [формулу](article|~/factorial|factorial-recurrent) факториала:
...
@problem
    ...
    solution:
        ...
        "Вся доска": |
            ...
            Если бы фигуры были уникальными, можно было бы воспользоваться [формулой](article|~/arrangement|a-unique-factorial-formula) размещений без повторений:

            $$ A_{64}^{16} = \frac{64!}{48!} $$

            Но здесь можно провернуть хитрый финт.
            Можно добавить к $16$ фигурам $48$ пустых клеток и думать о них, как о еще $48$ повторяющихся фигурах.
...
@problem
    ...
    hint: |
        Аналогичная [задача](practice|~/arrangement|wonder-calc) в теме [Размещения](page|article|~/arrangement).
    solution:
        "": |
        ...
        "Девять цифр": |
            ...
            Количество способов это сделать можно посчитать по [формуле](article|~/arrangement|a-unique-formula) размещений без повторений:
...
@problems
    ...
    level: hard
    shared: "Найдите сумму четырехзначных чисел, получаемых при всевозможных перестановках следующих $4$ цифр:"
    set:
        -   description: |
                $$ 1, \ 2, \ 2, \ 5 $$
            hint: |
                Решается почти так же, как и [Комбинаторный сумматор](sum), но с небольшим нюансом.
            solution: |
                Как и в [Комбинаторном сумматоре](sum), нам нужно отдельно сложить все единицы, десятки, сотни и тысячи.
...
@problem
    ...
    hint: |
        Адаптируйте решение [задачи](practice|~/arrangement|number-composition) о композициях числа из статьи про размещения.
    solution: |
    ...
    note: |
        Эта задача является более конкретным вариантом замечательной [задачи](practice|~/arrangement|number-composition) о композициях числа.
...
@problem
    ...
    solution: |
        Отдельно [посчитайте сумму](sum) всех чисел, в том числе когда $0$ является первой цифрой.
...
@problem
    ...
    note: |
        ...
        Кстати, такой же подход можно использовать и в задаче [Званый вечер](nice-evening).
...
@problem
    ...
    description: |
        ...
        @image
            ...
            maxWidth: 150px
    hint: |
        Для условия с поворотом квадрата воспользуйтесь рассуждениями из решения задачи [Хочешь в ряд, а хочешь в круг](row-circle).
    solution: |
        ...
        Точно так же, как и в задаче [Хочешь в ряд, а хочешь в круг](row-circle), если допустить вращение квадрата, то среди $45 \ 360$ перестановок оказывается множество дубликатов.
