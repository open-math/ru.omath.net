В [предыдущей теме](page|article|../../definition) для записи дробей мы использовали либо частично закрашенные круги, либо фразы по типу "3 части из 8".
...
@term
    ...
    main: |
        Дробь, записанная с использованием дробной черты. В общем виде $a/b$ или $\small\dfrac{a}{b}$.
        Число $b$ называется **знаменателем**{ accent } и означает, на сколько частей был поделен объект. Число $a$ называется **числителем**{ accent } и означает, сколько частей было "взято".
...
@term
    ...
    main: |
        Обыкновенная дробь, у которой числитель меньше знаменателя.
...
{ #inproper-fraction +toc }
@term
    title: Неправильная дробь
    main: |
        Обыкновенная дробь, у которой числитель равен или больше знаменателя.
...
Например, дробь $6/3$ [означает](why) целое число $2$.
