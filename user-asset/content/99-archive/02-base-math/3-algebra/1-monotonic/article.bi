...
@term
    ...
    main: |
        Функция $f(x)$ на промежутке $M\subseteq \R$ называется **возрастающей**{ accent } или **неубывающей**{ accent }, когда для любых двух чисел $x_1, x_2 \in M$ этого промежутка, таких, что $x_1 < x_2$, выполняется неравенство $f(x_1) \leq f(x_2)$.
...
{ #increasing-strict-connection +toc }
@statement
...
@term
    ...
    main: |
        Функция $f(x)$ на промежутке $M\subseteq \R$ называется **убывающей**{ accent } или **невозрастающей**{ accent }, когда для любых двух чисел $x_1, x_2 \in M$ этого промежутка, таких, что $x_1 < x_2$, выполняется неравенство $f(x_1) \geq f(x_2)$.

        $$ \forall x_1, x_2 \in M \ : \ x_1 < x_2 \Rightarrow f(x_1) \geq f(x_2) $$

{ #decreasing-strict +toc }
@term
    title: Строго убывающая функция
    main: |
        Функция $f(x)$ на промежутке $M\subseteq \R$ называется **строго убывающей**{ accent }, когда для любых двух чисел $x_1, x_2 \in M$ этого промежутка, таких, что $x_1 < x_2$, выполняется неравенство $f(x_1) > f(x_2)$.
...
@example
    ...
    sections:
        Общее: |
            Во всех пунктах, кроме последнего, ход доказательства аналогичен доказательствам в [примере](increasing) на неубывание.
...
@statement
    ...
    sections:
        proof: |
            Доказательство проводится [аналогичным образом](increasing-strict-connection).
...
@term
    ...
    main: |
        **Монотонная функция**{ accent } — неубывающая или невозрастающая функция.

{ #monotonic-strict +toc }
@term
    title: Строго монотонная функция
    main: |
        **Строго монотонная функция**{ accent } — строго возрастающая или строго убывающая функция.
...
@example
    ...
    sections:
        solution: |
            По [определению](article|../abs|abs) функции модуля:
...
@statement
    ...
    sections:
        proof: |
            ...
            Константу можно представить в виде константной функции. Как мы уже выяснили в [примере](const) выше, любая константная функция одновременно является невозрастающей и одновременно неубывающей.
...
@example
    ...
    sections:
        solution: |
            Константа $B$ [не влияет](monotonic-sum) на монотонность $f(x)$, поэтому ее можно отбросить.
            Остается только $Ax$. Выше мы [выяснили](increasing), что функция $x$ строго возрастает на всей своей области определения.
            Значение константы $A$ на монотонность [не влияет](monotonic-product) (при условии, что $A\neq 0$). На нее влияет только знак этой константы.
...
@example
    ...
    sections:
        solution: |
            Сразу выбрасываем константу $C$ в окно, так как она [не влияет](monotonic-sum) на монотонность $f(x)$.
            ...
            Константа $C'$ [не влияет](monotonic-sum) на монотонность, поэтому можно ее отбросить.
            ...
            Будем считать, что $A\neq 0$, так как в противном случае из квадратинчой функции получим линейную. Если константа $A$ положительная, то она [не повлияет](monotonic-product) на монотонность. Если отрицательная — монотонность [изменится](monotonic-product) на противоположную.
...
Из [определения](monotonic) мы знаем что функцию называют монотонной, если она является неубывающей или невозрастающей, то есть для любой пары функция неубывает или для любой пары невозрастает.
...
@example
    ...
    sections:
        solution: |
            ...
            По [определению](non-monotonic) для доказательства немонотонности $f(x)$ нам нужно найти две пары, на одной из которых она строго возрастает, а на другой строго убывает.
            ...
            В первых двух вариантах сходу получаем противоречие, ведь на этих промежутках $f(x)$ строго монотонна, а значит $f(x_1)$ не может равняться $f(x_2)$.
            ...
            В то же время, на отрезке $[1,2]$ функция $f(x)$ строго убывает, а значит [имеет](monotonic-min-max) единственный минимум в точке $2$, равный $f(2) = -2+3=1$. В худшем случае получаем, что $x_2 = 2$, а значит $f(x_2) = 1$. И даже в этом худшем случае получаем, что $f(x_1) < f(x_2)$.
...
@statement
    ...
    sections:
        proof: |
            ...
            По [теореме](monotonic-inverse) о монотонности обратной функции у функции $f$ на можестве $M$ есть строго возрастающая обратная функция $f^{-1}$. Тогда для любой пары значений функции $f$, в том числе
...
@example
    ...
    sections:
        solution: |
            ...
            Теперь воспользуемся [теоремой](monotonic-min-max) о минимумах и максимумах. Раз $3|x| - 100$ строго убывает на промежутке $(-\infty, 0]$, то в точке $0$ эта функция имеет единственный минимум. К аналогичному результату приходим на промежутке $[0, +\infty)$, на котором функция строго возрастает.
...
@example
    ...
    sections:
        solution: |
            ...
            Из [упражнения](decreasing) по исследованию на невозрастание вспоминаем, что функция $\frac{1}{x}$ строго убывает на положительной полуоси, а значит и на отрезке $[1,5]$.
            Из [ислледования](increasing) на неубывание помним, что $x^2$ строго возрастает на $[0, +\infty)$, а значит и на $[1,5]$. Но в нашем случае мы умножаем эту функцию на $-1$,
            что по следствию из [теоремы](monotonic-product) о произведении монотонных функций означает, что $-x^2$ строго убывает на $[1,5]$.

            По [теореме](monotonic-sum) о сумме монотонных функций сумма строго убывающих функций $\frac{1}{x}$ и $-x^2$ тоже будет строго убывать на $[1,5]$.
            Наконец, из [теоремы](monotonic-min-max) о минимумах и максимумах следует, что у рассматриваемой функции есть единственный максимум в точке $1$ и минимум в точке $5$.
