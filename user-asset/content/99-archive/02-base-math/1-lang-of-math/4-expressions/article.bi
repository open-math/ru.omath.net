...
В [прошлой статье](page|article|../alphabet) мы составили базовый алфавит математики. Буквы и знаки алфавита можно комбинировать, получая слова.

{ #word +toc }
@term
    title: Слово
    main: |
        Конечная последовательность (набор) знаков алфавита.
...
@term
    title: Выражение
    main: |
        Слово на языке математики, которое имеет смысл.
...
@term
    title: Терм
    main: |
        1. Каждая отдельная цифра или буква латинского алфавита — **терм**{ accent }. Такие термы называются элементарными.
