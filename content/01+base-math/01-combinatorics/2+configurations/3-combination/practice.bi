...
~ combinations article|combination-formula
~ combinationsRepeat article|combination-repeat-formula
...
@problem
    ...
    hint: |
        Используйте [формулу](~combinations) сочетаний без повторений.
    solution: |
...
@problem
    ...
    description: |
        ...
        А сколькими способами можно составить команду из $4$ человек для участия в так называемой [шведской эстафете](run-explain) 100 + 200 + 400 + 800?

        { #run-explain title="Шведская эстафета" }
        @details
    ...
    solution: |
        ...
        Находим количество размещений по [формуле](article|~/arrangement|a-unique-factorial-formula):
        ...
        Но в каждом этом составе есть $4!$ способа назначить участникам дистанции ([формула](article|~/permutation|permutation-formula) перестановок).
...
@problem
    ...
    description: |
        ...
        Сколькими способами можно выбирать из неё в течение двух вечеров по $6$ человек для участия в спектаклях так, чтобы эти составы не совпадали друг с другом?
    hint: |
    ...
    solution: |
        ...
        Найдём количество составов по [формуле](~combinations): $C_{10}^6$.
...
@problem
    ...
    solution:
        ...
        "Через правило умножения": |
            ...
            Но каждая из этих точек пересечения [имеет](duplicate-explain) свой дубликат для случая, когда мы выбрали те же две прямые, но в обратном порядке.
            ...
            @details
                ...
                Подробнее про возникновение дубликатов с пояснениями и рисунками можно почитать в решении [задачи](practice|~/rules/product|diagonal-method) про нахождение диагоналей $n$-угольника.
...
@problem
    ...
    solution:
        ...
        "Через перестановки с повторениями": |
            ...
            Находим количество этих вариантов по [формуле](article|~/permutation|permutation-repeat-formula):
...
@problem
    ...
    level: medium
    description: |
        Олег решил задачу [Фруктовые дни](fruits) следующим образом.
...
@problem
    ...
    level: medium
    description: |
        Пять девушек и трое юношей играют в [городки](gorodki).
        ...
        { #gorodki title="Городки" }
        @details
            [Городки](https://w.wiki/8zCR) -- старинная русская народная спортивная игра.
...
@problem
    ...
    solution: |
        ...
        Разберём вариант, когда члены президиума могут быть и членами делегации.
        Есть $C_{52}^5$ способов выбрать членов президиума и вне зависимости от выбранных людей есть ещё $C_{52}^3$ способов выбрать членов делегации.
...
@problem
    ...
    description: |
        ...
        @image
            ...
            maxWidth: 675px
            caption: Примеры распределения героев по командам в трех матчах
    ...
    hint: |
        ...
        Решение немного схоже с решением [задачи](theatre) про артистов с той лишь разницей, что во второй команде (состав на второй вечер) не могут использоваться те герои (артисты), которые уже есть в первой команде (состав на первый вечер).
    solution: |
...
@problem
    ...
    solution: |
        ...
        Чтобы две диагонали смогли пересечься, они обязательно [должны быть](diagonals-explain) построены на $4$ **разных** вершинах $n$-угольника!
...
@problem
    ...
    solution:
        В порядке возрастания: |
            ...
            Далее, порядок выбранных $5$ цифр из $9$ не имеет значения, потому что выбранные цифру в порядке возрастания можно расположить [только одним](one-order-explain) способом.
            ...
            @details
                ...
                Разница между этими наборами будет в переставленных местами как минимум двух элементов, которые мы назовём $a$ и $b$.
...
@problem
    ...
    level: hard
    description: |
        На первой из двух параллельных прямых лежит $10$ точек, на второй -- $20$.
...
@problem
    ...
    solution: |
        ...
        **Разберём первое распределение.**
        В нём у нас $3$ карты одной масти.
        ...
        Каждую из оставшихся трёх карт разных мастей можно выбрать $13$ способами, то есть всего $13\cdot13\cdot13 = 13^3$ вариантов.
        ...
        **Теперь второе распределение.**
        В нём по две карты двух мастей.
        ...
        Нам подойдёт любой вариант как из первого подхода, так и из второго.
...
@problem
    ...
    hint: |
        Это копия задачи [Разные кости судьбы](practice|~/arrangement|soul-dices), поэтому методика решения схожа, только вместо размещений нужно использовать сочетания.
    solution: |
        Так как все кости одинаковые, нам абсолютно не важен порядок, в котором они выпадают.
        Поэтому мы будем использовать формулы сочетаний, а не размещений, как в аналоге этой задачи: [Разные кости судьбы](practice|~/arrangement|soul-dices).
    ...
    set:
        -   label: Образец
            ...
            solution: |
                ...
                Порядок, в котором мы назначим единицам переменные не важен.
                Пример:
        ...
        -   label: Отработка
            generator: generators/analyze-equation
            description: |
                Сколько решений в неотрицательных целых числах имеет следующее уравнение?
...
@problem
    ...
    hint: |
        Адаптируйте решение задачи [Анализ уравнений](roots) или используйте [метод перегородок](article|split-method).
    solution:
        "Через сочетания с повторениями": |
            ...
            Найдём количество таких сочетаний по формуле:

            $$ \bar{C}_m^n = C_{n+m-1}^n = \frac{(n+m-1)!}{n! \ (m-1)!} $$

        "Через перегородки": |
            Задачу можно решить и [методом перегородок](article|split-method).
            Распишем число $n$ в виде $n$ единиц:
...
@problem
    ...
    solution: |
        ...
        Найдём их количество по [формуле](~combinationsRepeat):
        ...
        Так как каждая буква встречается среди этих сочетаний [одинаковое](repeat-count-explain) количество раз, то делим это выражение на количество букв ($n$) и получаем количество раз, которое встретится каждая буква:

        { #repeat-count-explain }
        @details
            Возьмём две любые буквы из сочетаний, например $a$ и $b$.
            На любую комбинацию, в которой есть буква $a$, найдётся точно такая же комбинация, где вместо $a$ там стоит $b$.
