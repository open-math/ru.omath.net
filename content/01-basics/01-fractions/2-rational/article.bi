...
{ #negative-portions }
Итак, наличие одного минуса у долей означает долг, отрицательную "часть от целого", "целое" или "целое с частями".
Если минуса два, это всё равно что своровать сворованное -- как-будто доли без минусов вовсе -- положительную "часть от целого", "целое" или "целое с частями".
...
{ #number-ray-orientation-examples }
@example
...
~> practice|general-knowledge-practice
Сейчас лучше сделайте остановку.
Прежде чем двигаться дальше лучше отработайте до автоматизма в практикуме ваше понимание долей, обыкновенных дробей и рациональных чисел.

# Дробей много -- суть одна
...
$$ 1 = \frac{1}{1} = \frac{2}{2} = \frac{1000}{1000} $$
...
Можно брать куски одинакового размера и получать "равные" дроби.
...
@example
    ...
    main: |
        При помощи стены дробей найдите по две одинаковые дроби к следующим дробям:
...
Итак, рациональное число может иметь разные записи в виде обыкновенных дробей, но обозначать "одно и то же" целое или часть от целого.
Логичный следущий вопрос -- а можно ли как-то быстро и без вандализма с порчей стен из одной дроби получить другую дробь, обозначающую то же самое?
...
$$ \frac{1}{2} = \frac{1 \brand{\times 3}}{2 \brand{\times 3}} = \frac{3}{6} $$
...
Получается, умножив числитель и знаменатель дроби на **одно и то же число**, мы получаем новую дробь из **большего** количества долей, которая обозначает то же, что и исходная дробь:
...
$$ \frac{4}{6} = \frac{4 \yellow{: 2}}{6 \yellow{: 2}} = \frac{2}{3} $$

Получается, поделив числитель и знаменатель дроби на **одно и то же число** мы получаем новую дробь из **меньшего** количества долей, которая обозначает то же, что и исходная дробь:
...
{ #fraction-basic-property link }
@statement
    title: Основное свойство дроби
    main: |
        Если числитель и знаменатель обыкновенной дроби умножить или разделить нацело на **одно и то же натуральное число**{ +accent }, то получится новая дробь, обозначающая то же самое целое/часть от целого/целое с частью, что и исходная дробь.
...
1. Через него строго определяется, что значит **равные** или **эквивалентные** рациональные числа.
2. Позволяет упрощать дроби, записывать их с меньшим количеством долей, что делает их более удобными для восприятия и работы.
3. Используется при доказательстве теорем и формул, связанных с дробями.
4. Позволяет [сравнивать](article|../actions|comparison) разные дроби (определять, какая из двух больше, меньше).
5. [Сложение и вычитание](article|../actions|sum-diff) дробей возможно только благодаря этому свойству.

Последние два пункта мы в подробностях разберем в [следующей теме](page|article|../actions), посвященной действиям с дробями.
Не просто же так мы их придумывали, чтобы потом ничего с ними не делать!
А вот с первыми тремя разберемся в этой теме.

## Эквивалентные дроби

До этого момента мы говорили про "равные" и "одинаковые" дроби в интуитивном смысле, мол это те дроби, которые обозначают одно и то же целое или часть от целого.
Теперь пришло время дать строгое определение, что понимается под "равными" или "эквивалентными" рациональными числами.
Не пугайтесь этого сложного слова.
"Эквивалентный" -- это у математиков заумный способ сказать "равный" или "одинаковый":

{ #equal-rationals link }
@term
    title: Эквивалентные дроби
    main: |
        Два рациональных числа называют **равными**{ +accent } или (заумно) **эквивалентными**{ +accent }, если одно число из другого можно получить по [основному свойству дроби](fraction-basic-property) -- домножением или сокращением на одно и то же натуральное число.
...
Как видите, это определение сохраняет интуитивный смысл, которым мы пользовались раньше, но при этом становится строгим и на него можно ссылаться в доказательствах.

В отличие от целых чисел, у которых на каждое число всего одна запись (например $3 = 3$), у любого рационального числа есть **бесконечное** количество **равных, но разных** записей в виде обыкновенных дробей:

$$ \frac{1}{2} = \frac{2}{4} = \frac{3}{6} = \frac{100}{200} = \frac{9999}{19998} = \frac{123 \ 456}{246 \ 912} = \cdots $$
...
@example
    ...
    main: |
        ...
        Приведите несколько эквивалентных дробей.
    sections:
        solution: |
            ...
            И числитель и знаменатель этой дроби делятся на $2$ нацело, поэтому по [основному свойству дроби](fraction-basic-property) можно сократить эту дробь на $2$:
            ...
            Это логично, $30$ рублей из $120$ рублей это действительно четверть всей суммы, что и отражает полученная дробь!
            Получается, долг Яны можно выразить следующими дробями:
...
В практикуме вы поработаете с несократимыми дробями [не раз](practice|irreducible-evens), [не два](practice|irreducible-exists) и даже [не три](practice|irreducible-unique)...
...
Но это уже более продвинутая тема, но безумно интересная!
...
{ #fraction-simplification-compact }
@image
...
Если быть уже совсем честным, иногда [сокращать можно и в случае сложения/вычитания](practice|complex-fraction-simplification), но в довольно специфических ситуациях, которые еще и надо уметь быстро подмечать.
Если вы только осваиваете дроби, лучше придерживайтесь "железного правила сокращения".
