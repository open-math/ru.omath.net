...
@term
    title: Модуль числа
    main: |
        **Модуль**{ accent } или **абсолютная величина**{ accent } вещественного числа $x$ — само число $x$, если оно неотрицательно, иначе $-x$.
...
@statement
    ...
    sections:
        proof: |
            ...
            Если $x< 0$, то неравенство $x\leq |x|$ выполняется, так как слева имеем отрицательное число, а число справа всегда [неотрицательное](abs-positive).
...
@statement
    ...
    sections:
        proof: |
            Из [свойства](abs-inverse-equality) равенства модулей противоположных чисел все равенства ниже выполняются:
...
@statement
    ...
    sections:
        proof: |
            Слева и справа имеем положительные числа, поэтому можно возвести в квадрат обе части неравенства и воспользоваться [свойством](abs-pow) связи модуля и квадрата:
            ...
            Для правой части воспользуемся [свойством](abs-product) произведения модулей:
            ...
            Оба неравенства всегда выполняется. Первое выполняется, так как модуль всегда [не меньше](abs-arg) аргумента. Второе выполняется по этому же свойству, а также из-за того, что модули противоположных чисел [равны](abs-inverse-equality).
...
@statement
    ...
    sections:
        proof: |
            Если $|x|-|y|$ меньше $0$, то неравенство выполняется, так как модуль всегда [неотрицателен](abs-positive).

            Пусть тогда $|x| - |y| \geq 0$. Возведем обе части неравенства в квадрат и воспользуемся [свойством](abs-pow) связи модуля и квадрата:
            ...
            Это неравенство всегда выполняется (см. [доказательство](abs-triangle-ineq) неравенства треугольника).
...
{ #distance-between-dots +toc }
@term
    title: Расстояние между точками
    main: |
        **Расстоянием**{ accent } между двумя точками $a$ и $b$ на числовой оси называется модуль их разности: $|a-b|$.
...
@term
    ...
    main: |
        **Модуль**{ accent } (функция) — кусочно-линейная функция, определенная на всей вещественной прямой $\mathbb{R}$ следующим образом:

        $$ |x| = \begin{cases} \phantom{-}x, \quad x \geq 0 \\ -x, \quad x < 0 \end{cases} $$

Видим, что механизм получения значения полностью совпадает с [определением](abs) модуля числа, которое я дал в начале статьи. Никакой разницы в значениях между этими определениями нет. Это означает, что все выведенные выше свойства модуля числа прекрасно сохраняются и для функции модуля.
...
@statement
    ...
    sections:
        proof: |
            ...
            Для любых неотрицательных чисел функция модуля будет равна самим этим числам. Значит, любое положительное число входит в область значений функции модуля. В то же время, модуль всегда [неотрицателен](abs-positive), поэтому отрицательных чисел в области значений не может быть. Значит, область значений равна $[0, +\infty)$.

            Доказанное ранее [свойство](abs-inverse-equality) равенства модулей противоположных чисел и означает четность функции модуля по определению.
...
@statement
    ...
    sections:
        proof: |
            Докажем сначала для случая, когда $g < 0$. В этом случае, какое $f$ ни возьми, его модуль всегда будет [положительным](abs-positive), поэтому не существует такого числа $f$, при котором получили бы равенство $|f| = g$.

            Пусть теперь $g\geq 0$. Тогда мы можем возвести обе части равенства в квадрат и воспользоваться [свойством](abs-pow) связи модуля и квадрата:
...
@statement
    ...
    sections:
        proof: |
            Стоит сразу отметить, что если $g < 0$, то неравенство $|f| < g$ не выполняется вне зависимости от значения $f$, ведь модуль всегда [неотрицателен](abs-positive) и мы получаем, что положительное число $|f|$ должно быть меньше, чем отрицательное $g$, чего не может быть.

            Получается, рассматривать неравенство $|f| < g$ имеет смысл только при $g \geq 0$. Тогда возведем обе части неравенства в квадрат и воспользуемся [свойством](abs-pow) связи модуля с квадратом:
...
@statement
    ...
    sections:
        proof: |
            Сразу заметим, что если $g < 0$, то неравенство выполняется вообще всегда, вне зависимости от $f$, ведь модуль всегда [неотрицателен](abs-positive).

            Разберем вариант, когда $g \geq 0$. Тогда мы, прямо как и для [неравенств](abs-ineq-less) вида $|f| < g$, можем возвести обе части неравенства в квадрат и воспользоваться [свойством](abs-pow) связи модуля с квадратом:
