...
Чтобы не тратить время и не путаться, введём обозначение $S_n^k$, обозначающее сумму $k$-ых степеней первых $n$ натуральных чисел:

{ #sum-of-powers +toc link }
@term
...
@example
    ...
    main: |
        ...
        @math
            S^0_3 = \ ? >> S^1_5 = \ ? >> S^2_{10} = \ ? >> S^k_8 = \ ? >> S^3_n = \ ?
    sections:
        solution: |
            @math
                S^0_3 = 1^0 + 2^0 + 3^0
...
@important
    title: Наша задача
    main: |
        В этой теме мы попытаемся **найти прямые и удобные формулы**{ accent } для расчёта произвольных $S_n^k$ -- сумм натуральных чисел в любой степени.
...
Начнём с сумм вида $S_n^0$, то есть сумм первых $n$ натуральных чисел в нулевой степени:
...
Сумма двух единиц это число $2$. Трёх -- $3$.
Сумма $n$ единиц -- число $n$.

{ #sum-of-ones-formula +toc link }
@statement
    title: Сумма единиц
    main: |
...
Одно интересное применение мы рассмотрим тут, а другие дожидаются вас в [практикуме](page|practice).

Но сначала надо разобраться, а как такие суммы вообще считать!

## Гениальное решение

Согласно легенде, $8$-летний Карл Гаусс плохо себя вёл на уроке.
...
Выпишем в один ряд числа от $1$ до $100$, а под ним ещё один ряд тех же чисел, но в обратном порядке -- от $100$ до $1$.
...
Удивлять всё математическое сообщество невероятными результатами будущий [король математики](https://w.wiki/9SuP) будет на протяжении всей своей жизни...
...
Будем делать это зелёными квадратами.
...
Прямоугольник мы построили из одинакового количества синих и зелёных квадратов.
Поделив найденную площадь на $2$, мы как раз и найдём количество только синих (или только зелёных) квадратов, а значит и искомую сумму $n$ первых натуральных чисел:
...
@callout
    ...
    content: |
        ...
        Например, на складе брёвен достаточно места, чтобы выложить на земле $22$ брёвна друг рядом с другом.
        Какое максимальное количество брёвен можно хранить на таком складе?

        Брёвна надо укладывать в форме треугольника, иначе они укатятся.
...
@important
    main: |
        Это решение поначалу может показаться непонятным и запутанным, но обязательно разберитесь в нём!
        В конечном итоге, когда закончатся геометрические аналогии и гениальные озарения, останется **только этот**{ accent } вариант решения!

Начнём со следующего равенства, в котором мы используем [формулу](diff-2) квадрата разности:

{ #diff-2 }
@details
    Если вы по какой-то невероятной причине забыли эту формулу, то её всегда можно вывести вручную, умножив друг на друга две одинаковые скобки:
...
Заменяем длинные суммы на [введённое обозначение](sum-of-powers) для сумм степеней чисел:
...
Ранее мы уже выяснили, что $S_n^0$ [равно](sum-of-ones-formula) $n$.
Производим замену, а также изолируем сумму $S_n^1$, ведь именно её мы и ищем:

$$ S_n^1 = \frac{1}{2}\left( S_n^2 - S_{n-1}^2 + n \right) $$

Осталось только найти, чему равна разность $S_n^2 - S_{n-1}^2$.
Довольно [очевидно](sum-diff-explain), что если из суммы квадратов от $1$ до $n$ вычесть сумму квадратов от $1$ до $(n-1)$, то в итоге останется только $n^2$.
...
{ #sum-of-numbers-formula +toc link }
@statement
...
Повышаем ставки!
Теперь мы будем искать прямую формулу для расчёта суммы квадратов чисел:
...
Нужно попытаться достроить эту пирамиду до простой фигуры, объём которой легко найти. Например, до параллелепипеда.
...
Значит, для получения одной суммы, надо найденный объём поделить на $6$:

$$ 1^2 + 2^2 + 3^2 + \ldots + n^2 = \frac{n(n+1)(2n+1)}{6} $$

Вот так вот геометрия в очередной раз пришла на выручку алгебре!
Геометрическое решение хорошо ещё и тем, что с его помощью саму формулу легко запомнить: параллелепипед с увеличивающимися сторонами $n$, $n+1$, $2n+1$, делим на $6$ одинаковых пирамид, из которых он и состоит.

@callout
    icon: assets/cannon-cores.jpg
    title: Квадратные пирамидальные числа
    content: |
        Мы открыли ещё один тип так называемых "фигурных" чисел, то есть чисел, которые можно представить в виде геометрических фигур.
...
{ #sum-of-squares-formula +toc link }
@statement
    ...
    sections:
        proof: |
            Выпишем разложения $(n-1)^3$ по [формуле](diff-3) куба разности для всех чисел от $1$ до $n$:

            { #diff-3 }
            @details
                Представляем $(a-b)^3$ как произведение $(a-b)(a-b)^2$, скобку в квадрате заменяем по [формуле](diff-2) квадрата разности и проводим умножение вручную:
            ...
            Складываем все эти равенства и используем введённое [обозначение](sum-of-powers) для сумм:
            ...
            Подставляем уже найденные формулы для сумм $S_n^0$ и $S_n^1$, а также [заменяем](cube-diff-explain) разность $S_n^3 - S_{n-1}^3$ на $n^3$:

            { #cube-diff-explain }
            @details
                Тут ситуация точно такая же, как и с разностью $S_{n}^2 - S_{n-1}^2$ в [выводе](sum-diff-explain) формулы суммы натуральных чисел.
            ...
            Для получения запакованной формулы как в геометрическом решении приводим всё к общему знаменателю и находим корни квадратного уравнения:
...
@example
    ...
    sections:
        solution: |
            С помощью выведенной [формулы](sum-of-squares-formula) находим сумму всех квадратов чисел от $1$ до $50$:
...
Последняя степень, в которой ещё можно получить прямую формулу "элементарным" и наглядным способом.
Разберёмся же с суммой кубов чисел!

$$ S_n^3 = 1^3 + 2^3 + 3^3 + \ldots + n^3 = \ ? $$

## Гениальная гипотеза

Сейчас речь пойдёт не о решении, а о гипотезе.
Её плюс в том, что она очень легко запоминается и выводится, а минус в том, что процесс её получения не является доказательством -- верность полученной формулы придётся доказывать отдельно.
...
@callout
    ...
    content: |
        ...
        Причём последний слой каждого чётного куба нужно дополнительно разрезать надвое.

        Из полученных слоёв выкладываем большой квадрат со стороной, равной сумме чисел $1 + 2 + \ldots$
...
Гипотеза может выглядеть сколь угодно соблазнительно, но использовать её без доказательства опасно.
К счастью, у нас уже есть рабочий метод вывода прямых формул для сумм степеней чисел:

{ #sum-of-cubes-formula +toc link }
@statement
    title: Сумма кубов
    main: |
    ...
    sections:
        proof: |
            Повторяем точно такие же действия, как и при [выводе](sum-of-squares-formula) сумм квадратов.

            Для строгого вывода формулы нам нужно найти разложение $(n-1)^4$.
            Сделать это можно двумя способами: долго и муторно умножать скобки вручную или задействовать [бином Ньютона](article|/combinatorics/binomial-theorem|binom).
            Именно его и используем:

            $$ (n-1)^4 = C_4^0 n^4 - C_4^1 n^3 + C_4^2 n^2 - C_4^3 n + C_4^4  $$

            Считаем биномиальные коэффициенты либо по [формуле сочетаний](article|/combinatorics/combination|combination-formula), либо через [треугольник Паскаля](article|/combinatorics/binomial-theorem|pascal-triangle):

            $$ (n-1)^4 = n^4 - 4n^3 + 6n^2 - 4n + 1 $$

            Суммируем это равенство для всех чисел от $1$ до $n$ и полученные суммы заменяем на введённые [обозначения](sum-of-powers):
            ...
            Для получения запакованного варианта приводим к общему знаменателю, выносим за скобки $n^2$ и применяем формулу квадрата суммы к оставшейся скобке:
...
@example
    ...
    sections:
        solution: |
            Используем выведенную [формулу](sum-of-cubes-formula):
...
**Геометрический подход перестаёт работать.**
Для простой суммы чисел в первой степени мы задействовали площадь.
Для суммы квадратов задействовали объём.
...
**Хитрые приёмы и озарения тоже закончились.**
...
Вот и получается, что два самых любимых метода учёных древности (геометрия и необычные ходы) перестали работать.
...
Каждый раз, когда речь заходит о степенях и длинных суммах, это повод задуматься о [биноме Ньютона](page|article|/combinatorics/binomial-theorem).
...
Если у вас возникнут сложности, то сначала самостоятельно выведите уже описанные выше формулы для [первой](sum-of-numbers-proof), [второй](sum-of-squares-formula) и [третьей](sum-of-cubes-formula) степеней алгебраическим способом.
...
Теперь заменим сумму в левой части равенства и суммы в скобках в правой части на введённое [обозначение](sum-of-powers) сумм степеней чисел:
...
Но нам всё же нужна сумма $n$ чисел в $k$-й степени, а не сумма $n$ чисел $k+1$-й степени.
Поэтому ещё немного преобразуем равенство.

Для начала избавимся от сумм $k+1$-й степени, используя тот факт, что разность $S_n^{k+1} - S_{n-1}^{k+1}$ [равна](diff-general-explain) $n^{k+1}$:
...
@example
    title: Покорение четвёртой степени!
    main: |
    ...
    sections:
        solution: |
            Пользуемся выведенной рекуррентной [формулой](recurrent-formula):
            ...
            Формула готова!
            С её помощью считаем сумму первых $15$ чисел в четвёртой степени:

            $$ S_{15}^4 = \frac{1}{5}\cdot 15^5 + \frac{1}{2}\cdot 15^4 + \frac{1}{3}\cdot 15^3 - \frac{1}{30}\cdot 15 = 178 \ 312 $$

# Прямая формула?

Полученной нами рекуррентной [формулы](recurrent-formula) вполне достаточно для вывода прямых формул сумм чисел небольших степеней.
Но чем выше степень, тем более трудоёмким становится процесс вывода.
