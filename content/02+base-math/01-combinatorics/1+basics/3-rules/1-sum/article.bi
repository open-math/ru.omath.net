...
* В группе $G_2$ все способы осветить команту **двумя зажженными свечами**. Количество способов зажечь две свечи из четырех [можно найти](candles-2-explain) прямым перебором. Всего их $6$.
* В группе $G_3$ все способы осветить команту **тремя зажжеными свечами**. Так же прямым перебором [находим](candles-3-explain) $4$ варианта это сделать.
...
@details
    ...
    Обратите внимание, что таким образом мы выполнили [оба правила](article|~/brute-force|brute-force-rules) прямого перебора, чем сильно упростили себе работу.
...
@diagram
    graph TD
        root{{Способы освещения команты}} -->|$1$ свечой| g1["$G_1 = 4$"]
        root -->|$2$ свечами| g2["$G_2 = 6$"]
        root -->|$3$ свечами| g3["$G_3 = 4$"]
        root -->|$4$ свечами| g4["$G_4 = 1$"]
        class g1,g2,g3,g4 fill
...
[Отвлекаясь от смысла](article|~/preface|cut-the-crap), мы все возможные варианты **разбиваем** удобным нам образом **на группы**.
    ...
    sections:
        solution: |
            ...
            * Группа "Четные". В нее входят $3$ буквы: "т", "о", "к"
            ...
            Казалось бы, получили две группы вариантов. Осталось применить [правило сложения](rule-of-sum) и получить ответ:
...
{ link="Необычное применение сложения" }
# Цепочка действий
...
@example
    ...
    sections:
        "Решение от «конца к началу»": |
            ...
            Попасть в $E$ можно только из точек $B$, $C$ и $D$, причём только одним способом (одной дорогой) из каждой.
            Значит, каждый способ попасть, например, в $B$ автоматически [является способом](why-every-road) попасть и в $E$!
            Если в $B$ можно попасть, условно, $5$-ю способами, то это сразу даёт $5$ способов попасть и в $E$.

            { #why-every-road }
            @details
                Предположим, что в точку $B$ можно попасть $5$-ю способами.
                Из точки $B$ в $E$ ведёт всего одна дорога.
                Поэтому любой из $5$-ти способов попасть в $B$, пройдя ещё по одной единственной дороге, превращается в способ попасть в $E$.
            ...
            Плюс ещё и в том, что эти классы не пересекаются, ведь маршруты в них уникальны, со своей предпоследней точкой.

            Итак, вопрос о попадании в точку $E$ сводится к вопросу о всех способах попасть в "точку $B$ ИЛИ точку $C$ ИЛИ точку $D$".
            А на такие вопросы можно ответить с помощью [правила суммы](rule-of-sum)!
            Введём обозначение $S_E$, которое обозначает количество способов попасть в точку $E$.
            Тогда:
        ...
        "Решение от «начала к концу»": |
            ...
            Этот важный вывод позволяет просто двигаться от начала к концу и проставлять рядом с каждой точкой количество способов в неё попасть:
