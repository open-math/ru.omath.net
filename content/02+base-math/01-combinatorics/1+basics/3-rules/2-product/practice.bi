~ productRule article|rule-of-product
~ sumRule article|../sum|rule-of-sum
...
{ #photo-session }
@problem
    title: Памятная фотосессия
    level: medium
    attributes:
        - pretty
    description: |
        Несколько подруг решили обменяться фотографиями на память.
        Чтобы каждая девочка получила по одной фотографии своей подруги, потребовалось $30$ фотографий.
        Сколько было подруг?
    hints:
        - Обозначьте количество подруг за $x$.
        - Сколько есть способов выбрать подругу-фотографа? Сколько фоток эта подруга-фотограф должна сделать?
    solution: |
        Обозначим количество подруг за $x$.

        Сначала выбираем подругу, которая будет делать фотографии.
        Это можно сделать $x$ способами.
        Вне зависимости от выбранной подруги-фотографа, она сделает $x-1$ фотографий (всех подруг, кроме себя).

        Так как количество снимков не зависит от выбранного фотографа, общее количество снимков всей фотосесии по [правилу умножения](~productRule) будет равно $x \cdot (x-1)$.
        Но нам уже известно, что всего было сделано $30$ фотографий, поэтому мы можем записать уравнение:

        $$ x \cdot (x-1) = 30 $$

        Вычитаем из обеих частей равенства $30$:
        ...
        Это [квадратное уравнение](page|article|/basics/equations/quadratic).
        Решаем его и находим два корня:

        $$ x_1 = 6 >>{big} x_2 = -5 $$

        Отрицательное количество подруг не имеет смысла, поэтому остается только вариант с $6$ подругами.
    answer: |
        $6$ подруг.
...
@problem
    ...
    description: |
        ...
        Сколько участников было на турнире?
    hints:
        - Задача решается почти так же, как и задача [Памятная фотосессия](photo-session).
        - Избавьтесь от "повторяющихся" партий, ведь партия, когда игрок $A$ играет с игроком $B$, это та же самая партия, когда игрок $B$ играет с игроком $A$!
    solution: |
        Повторим решение задачи [Памятная фотосессия](photo-session).
        Обозначаем количество участников турнира за $x$.

        Первого игрока можно выбрать $x$ способами.
        Вне зависимости от выбранного игрока, соперника ему можно подобрать $x-1$ способами (все игроки, кроме уже выбранного).
        По [правилу умножения](~productRule) общее количество сыгранных партий будет равно $x \cdot (x-1)$.
        Но нам уже известно, что всего было сыграно $45$ партий, поэтому мы можем записать уравнение:

        $$ x \cdot (x-1) = 45 $$

        Однако, при таких рассуждениях мы посчитали каждую партию дважды: когда игрок $A$ играет с игроком $B$ и когда игрок $B$ играет с игроком $A$.
        Поэтому для избавления от "повторяющихся" партий, нам нужно поделить их количество на $2$:

        $$ \frac{x \cdot (x-1)}{2} = 45 $$

        Умножаем обе части равенства на $2$:

        $$ x \cdot (x-1) = 90 $$

        Вычитаем из обеих частей равенства $90$:
        ...
        Отрицательное количество участников не имеет смысла, поэтому остается только вариант с $10$ участниками шахматного турнира.
    answer: |
        $10$ участников.
...
@problem
    ...
    hint: |
        Для нахождения количества способов выбрать пару воинов можете использовать [прямой перебор](page|article|~/brute-force).
        Далее примените [правило умножения](~productRule).
    solution: |
        Медика можно выбрать $7$ способами.
        Командира -- $3$ способами.

        Количество способов выбрать двух воинов для отряда можно определить [прямым перебором](page|article|~/brute-force), где уже разбиралась аналогичная [задача](practice|~/brute-force|no-copy).
...
@problem
    ...
    level: medium
    description: |
        Как изменится результат задачи [Фантастическая четверка](fantastic-four), если в добровольцы запишется универсал, который может выполнить роль воина, медика или командира?
    hint: |
    ...
    solution:
        "Кризис правила умножения": |
            ...
            А правило умножения работает **только** тогда, когда каждый выбор [не влияет](article|rule-of-product-usage) на количество способов сделать следующий выбор!
...
@problem
    ...
    level: medium
    description: |
        Имеется ткань пяти различных цветов.
        Из ткани собираются делать флаги с тремя горизонтальными полосами.
        Сколькими способами можно сделать флаг с неповторяющимися цветами?
        А если цвета могут повторяться, но полосы должны быть различимы?
        А сколько существует разных флагов с красной полосой?
    ...
    hints:
        ...
        - Для флагов с красной полосой сформируйте три группы в зависимости от того, какая из трех полос красная.
    solution:
        "Все цвета разные": |
        ...
        "С повторяющимися цветами": |
            ...
            $$ 5 \cdot 4 = 20 $$
        "С красной полоской": |
            Если красная полоска расположена по центру, то верхнюю и нижнюю полоски можно раскрасить $4$-мя цветами (они могут быть даже одного цвета).
            Всего $4\cdot 4 = 16$ вариантов флагов с красной полоской по центру.

            Если красная полоска сверху, то имеем $4$ цвета для центральной полоски (все кроме красного) и $4$ цвета для нижней полоски (все, кроме той, в которую покрасили среднюю).
            Всего $4\cdot 4 = 16$ вариантов флагов с красной полоской сверху.

            Если красная полоска снизу, то по такой же логике вновь получаем $16$ вариантов флагов.

            Мы получили три группы флагов: "красная сверху", "красная по центру", "красная снизу".
            Группа "красная по центру" не пересекается с двумя другими, потому что у них красной полоски по центру быть не может.

            А вот группы "красная сверху" и "красная снизу" пересекаются в тех флагах, где красные полоски и снизу и сверху.
            Таких флагов всего $4$ (четыре варианта выбрать цвет средней полосы).

            Используем [правило сложения](~sumRule), не забыв вычесть $4$ дважды посчитанных флага:

            $$ 16 + 16 + 16 - 4 = 44 $$
    answer: |
        Всего $60$ флагов с различными цветами, $20$ флагов с повторяющимися цветами и $44$ флага с красной полоской.
...
@problem
    ...
    description: |
        ...
        Решите задачу в двух вариантах, когда цифры можно повторно использовать и когда нельзя.
    hints:
        - Число делится на $2$, если  последняя его цифра делится на $2$.
        - Когда цифры повторно использовать нельзя, начните с конца, ведь четность числа определяется именно последней цифрой.
        - |
            Когда цифры повторно использовать нельзя, сначала посчитайте (с конца) все пятизначные числа, даже "неправильные", начинающиеся с $0$.
            Потом отдельно посчитайте числа, начинающиеся с $0$ и вычтите их из общего результата.
    solution:
        "Цифры можно использовать повторно": |
        ...
        "Цифры повторно использовать нельзя": |
            Начнем с конца, ведь четность числа определяется именно последней цифрой.
            На первую цифру с конца в числе у нас есть всего $3$ кандидата: $0, 4, 8$.

            $$ \boxed{\phantom{0}} \cdot \boxed{\phantom{0}} \cdot \boxed{\phantom{0}} \cdot \boxed{\phantom{0}} \cdot 3 = \ ? $$

            На вторую цифру с конца может претендовать любое число, кроме уже использованного, то есть $5$ вариантов.
            На третью цифру с конца уже $4$, на четвертую с конца — $3$:

            $$ \boxed{\phantom{0}} \cdot 3 \cdot 4 \cdot 5 \cdot 3 = \ ? $$

            На пятую с конца, то есть на первую цифру числа, осталось лишь два кандидата.
            Да, может получиться так, что этой цифрой может оказаться $0$, но пока посчитаем вместе с ним:

            $$ 2 \cdot 3 \cdot 4 \cdot 5 \cdot 3 = 360 $$

            Всего $360$ "пятизначных" четных чисел, включая те, которые начинаются с $0$.

            Теперь отдельно посчитаем, сколько из $360$ чисел начинаются с $0$.

            Первой цифрой обязан идти $0$ (это единственный вариант), а последняя цифра должна быть четной (два варианта: $4$ или $8$):

            $$ \underbrace{1}_{0} \cdot \boxed{\phantom{0}} \cdot \boxed{\phantom{0}} \cdot \boxed{\phantom{0}} \cdot \underbrace{2}_{\text{4 или 8}} = \ ? $$

            Оставшиеся $4$ цифры можно раскидать как угодно:

            $$ 1 \cdot 4 \cdot 3 \cdot 2 \cdot 2 = 48 $$

            Теперь из всех "пятизначных" чисел ($360$) вычитаем четырехзначные ($48$) и получаем $360 - 48 = 312$ реальных пятизначных четных чисел.
    answer: |
        $3240$ пятизначных четных числа если цифры можно повторять. $312$ если цифры повторять нельзя.
...
@problem
    ...
    hint: |
        Для ответа на второй вопрос разбейте все варианты падения с условием "хотя бы два на грани с цифрой $2$" на классы. Обратите внимание на особый случай, когда все три волчка упали на грань с цифрой $2$.
    solution: |
...
@problem
    ...
    note: |
        ...
        Почему так происходит вы узнаете в похожей [задаче](practice|/applications/sum-of-powers|rectangles-in-square) на тему "[Суммы степеней чисел](page|article|/applications/sum-of-powers)".
...
@problem
    ...
    level: hard
    hint: |
        Задание решается практически точно так же, как и аналогичная [задача](practice|../sum|obedient-robot) в теме "Правило суммы".
    description: |
    ...
    solution: |
        ...
        Формулы для $R_n$ получатся почти такими же, что и в уже разобранной аналогичной [задаче](practice|../sum|obedient-robot).
...
@problem
    ...
    hint: |
        Задача решается почти так же, как и ее [аналог](practice|~/brute-force|latin-square) из темы "Прямой перебор".
    ...
    solution:
        ...
        "Заполняем таблицы": |
            ...
            Перестановки строк никак не ломают таблицу и [не могут](latin-square-swap) привести к возникновению повторяющихся букв в столбцах и строках.

            Перестановками строк можно получить $3 \cdot 2 \cdot 1 = 6$ разных комбинаций букв в левом столбце таблицы (буква $A$ закреплена в левом верхнем углу и не меняется). Объединяем этот результат с ранее получёнными $96$ закрасками и находим:
            ...
            { #latin-square-swap }
            @details
                Когда мы переставляем строки, их содержимое никак не меняется.
    ...
    note: |
        Эта и многие похожие задачи имеют общее название -- задачи о [латинском квадрате](https://w.wiki/88rj).
