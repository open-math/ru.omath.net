...
Сначала надо посетить звёздное скопление Омега Центавра, забрать груз и доставить на окраину галактики Андромеда.

Из Млечного Пути на Омегу Центавра есть $3$ основных звёздных маршрута.
...
Если мы полетим по верхнему пути до Омеги Центавра, то потом у нас будет $4$ варианта из неё попасть в Андромеду.
Но также $4$ варианта у нас будет, если до Омеги Центавра мы доберёмся по среднему пути, да и по нижнему тоже.
...
По цветам любой элемент одежды может быть чёрным, белым или же цветным.
Денег у Лены хватит только на покупку одной вещи.
Сколько разных вариантов закупиться у неё есть?
...
Но любой из этих $3$-х способов приведёт к $4$ возможным вариантам типа одежды.
$4$ типа чёрной одежды, $4$ белой и $4$ цветной.
...
Если нам нужно сделать два последовательных выбора, причём любой первый выбор не влияет на количество вариантов сделать второй выбор, то умножив количество вариантов первого выбора на количество вариантов второго мы получим все возможные комбинации совершить оба выбора.
...
На нём основан вывод многих других формул в этом разделе математики.

{ #rule-of-product +toc link }
@term
...
@example
    ...
    sections:
        solution: |
            ...
            По [правилу умножения](article|rule-of-product) получаем $5\cdot 6 = 30$ видов заборов.

# Летим дальше

Вернёмся к нашему комическому дальнобойщику.
Усложним ему жизнь, добавив ещё одну точку, которую он должен посетить -- созвездие Кассиопея.
Отправиться туда он должен из Андромеды, по двум возможным путям.

@image
    src: assets/galactic-trip-more.svg
    maxWidth: 650px

Попасть в Омегу Центавра он может тремя способами, и, вне зависимости от выбранного пути, после этого у него есть четыре пути в Андромеду. По [правилу умножения](rule-of-product) получаем всего $3 \cdot 4 = 12$ вариантов попасть в Андромеду.

Всего $12$ вариантов попасть в Андромеду. Каждый из этих $12$ вариантов превращается в $2$ способа попасть в Кассиопею. Берём "$12$ раз по $2$".
Получается всего $24$ варианта попасть в неё:
...
Этот пример отлично иллюстрирует, что [правило умножения](rule-of-product) работает не только для двух выборов, но вообще для любого их количества.

{ #generic-rule-of-product +toc link="Правило для цепочки выборов" }
@important
    title: Обобщённое правило умножения
    main: |
...
Каждый из $n_1$ способов порождает ещё $n_2$ способов выбрать элемент $a_2$.
То есть уже имеем $n_1 \cdot n_2$ способов выбрать $a_1$ и $a_2$.
Каждый из $n_1 \cdot n_2$ способов выбрать пару элементов $a_1$ и $a_2$ порождает ещё $n_3$ способов выбрать элемент $a_3$, что даёт в целом уже $n_1 \cdot n_2 \cdot n_3$ способов выбрать тройку элементов.
И так далее...

Строгое доказательство этого обобщённого правила [можно провести](proof-by-induction) методом математической индукции.
...
@details
    ...
    **Индукционный переход**
    ...
    Индукционный переход доказан, значит обобщённое правило умножения даёт верное количество комбинаций из любого числа элементов, при условии, что количество способов добавить новый элемент к комбинации не зависит от уже выбранных элементов.

    $\blacksquare$

Стоит также заметить, что правилом умножения (или правилом произведения) называют как само [определение](rule-of-product) для двух выборов, так и его [обобщённую версию](generic-rule-of-product).
...
Всё это время мы рассматривали задачи, которые можно за пару минут решить прямым перебором комбинаций.
...
@example
    ...
    main: |
        В ролевой компьютерной игре есть редактор, в котором игрок создаёт главного героя.
        Можно выбрать пол и одну из $4$ рас: человек, эльф, дворф или орк.
        Ещё можно выбрать один из $8$ оттенков кожи, $5$ вариантов лица, $20$ типов причесок.
    ...
    sections:
        solution: |
            ...
            Применяем [правило умножения](rule-of-product):
...
@example
    ...
    sections:
        solution: |
            ...
            По [правилу умножения](rule-of-product) версии получаем:
...
К счастью, существует простая техника записи правила умножения, которая всё сильно упрощает.
...
@example
    ...
    main: |
        ...
        Для этого пиццы распределяют между $5$ курьерами, причём каждый курьер доставляет только одну пиццу.
        Сколькими способами можно отправить пиццу заказчикам?
    sections:
        solution: |
            Доставить надо все $4$ пиццы, поэтому можно их изобразить как $4$ вакантных места в виде "кружочков" с обозначением $P_i$, где $P$ -- "пицца", а $i$ -- её номер.

            Первую пиццу можно отдать любому из пяти курьеров, поэтому в первом кружочке пишем $5$.
            Вне зависимости от первого выбора, вторую пиццу всегда можно отдать четырьмя способами -- любому из оставшихся четырёх курьеров, поэтому во втором кружке пишем $4$ и так далее.
...
@example
    ...
    main: |
        Сколько существует четырёхзначных чисел, в разряде десятков которых находится цифра $3$?
        Примеры:
    ...
    sections:
        solution: |
            ...
            Первая цифра числа [не может](first-digit) начинаться с $0$, поэтому в первый квадрат можно записать любую из $9$ оставшихся цифр.
            Во втором квадрате, то есть в разряде сотен, можно написать любую из $10$ цифр.
            В третьем квадрате всегда стоит цифра $3$, то есть в нём всегда используется одна цифра.
            Наконец, в последнем квадрате можно использовать любую из $10$ цифр:

            { #first-digit }
            @details
                Если в разряде тысяч стоит цифра $0$, то это уже не четырёхзначное число:
...
Заметили, что говоря о правиле умножения как в его [определении](rule-of-product), так и в примерах, постоянно повторяются слова "**при любом** выборе получаем $x$ способов сделать следующий выбор" и другие их вариации?
...
{ #rule-of-product-usage +toc link }
@important
    title: Применимость правила умножения
    main: |
        Правило умножения не работает, если **существуют**{ accent } выборы первого элемента, которые приводят к **разному**{ accent } количеству способов выбрать второй элемент!

Также, как и с [пересечениями](article|../sum|check-intersections) в правиле сложения, проверка на применимость правила умножения и разбирательство с последствиями лежит целиком на ваших плечах.
Причём в простых задачах уследить за этим элементарно.
