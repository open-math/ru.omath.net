...
* Мы ведь и зашли в этот учебник, чтобы изучать приёмы быстрого решения!

Эти и подобные вопросы и возмущения понятны.
Но прежде чем научиться летать, нужно сначала научиться ползать.
Прежде чем узнать о хитрых приёмах, надо сначала изучить и отработать базу -- перебор вариантов вручную.

Не волнуйтесь, у прямого перебора вариантов тоже есть свои хитрые приёмы и весьма интересные задачи.
Скучно не будет.

# Перебираем с умом

Казалось бы, что может быть проще!?
Просто берёшь и выписываешь все возможные комбинации!
...
Чтобы такого не происходило, нужно придерживаться двух простых правил:

{ #brute-force-rules +toc link }
@important
...
Именно так сущности вроде "яблок", "апельсинов" и "бананов" во время решения задачи превращаются в обезличенные буквы "я", "а" и "б".

Алгоритм перебора помогает не запутаться, не пропустить комбинации и не посчитать лишние.
При правильном выборе алгоритма решение задачи на перебор из пытки превращается в повторение чётких действий, которое неизбежно приведёт к правильным результатам.
...
@example
    ...
    main: |
        Пётр должен убраться в комнате, сделать домашнее задание и поработать над личным проектом.
        Сделать нужно всё, но Пётр ещё не решил, в какой последовательности.
    ...
    sections:
        solution: |
            ...
            Введя эти обозначения, мы выполнили первый пункт [правил перебора](brute-force-rules).
...
Поэтому им, обычно, не нужно придумывать уникальные обозначения и первое [правило перебора](brute-force-rules) можно считать автоматически выполненным.
...
@example
    ...
    sections:
        solution: |
            ...
            Введя подобные обозначения, мы выполнили первое [правило](brute-force-rules) прямого перебора.
...
@important
    ...
    main: |
        Если соблюдать [правила перебора](brute-force-rules), то множество комбинаторных задач можно решить полным (прямым) перебором.
...
@term
    ...
    main: |
        ...
        В этом алгоритме все варианты обозначаются в виде расходящихся "веток дерева".
...
В [первом примере](wonderful-day) этой статьи мы искали все способы составить расписание на день.
...
То же самое происходит и с оставшимися ветками первого уровня.
...
Причём для каждой из них эта буква всего одна.
...
{ #tree-variants }
@important
    title: Деревья могут быть любыми
    main: |
        Говоря о "деревьях" мы представляем себе растения, которые растут снизу вверх.
        Но в математике **форма и внешний вид деревьев не играют никакой роли**{ accent }.
...
Помните [задачу](hats) Эйлера про шляпы?
...
@example
    title: Полная шляпа
    main: Решите [задачу](hats) Эйлера о шляпах с помощью дерева вариантов.
    sections:
        solution: |
