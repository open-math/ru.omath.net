...
Количество таких [размещений без повторений](article|~/arrangement|a-unique) из $6$ человек по $6$ стульям можно посчитать по [формуле](article|~/arrangement|a-unique-formula):
...
Причём каждый элемент используется только один раз.
В этом смысле перестановку можно рассматривать как размещение без повторений.

{ #permutation-is-arrangement }
@important
    main: Перестановки -- **частный случай**{ accent } [размещений без повторений](article|~/arrangement|a-unique) из $n$ элементов по $n$ вакантным местам ($A_n^n$), то есть когда в размещении задействованы все имеющиеся элементы!
...
{ #permutation-formula +toc link }
@statement
    title: Формула перестановок
    main: |
        $$ P_n = n! $$
    sections:
        Через размещения: |
            Так как перестановки являются лишь частным случаем размещений без повторений, то для подсчета их количества будут работать соответствующие формулы, например [факториальная](article|~/arrangement|a-unique-factorial-formula):

            $$ P_n = A^n_n = \frac{n!}{(n-n)!} = \frac{n!}{0!} = \frac{n!}{1} = n! $$

            $\blacksquare$

            Если вас смущает, что факториал нуля вдруг оказался равен $1$, этот вопрос подробно и понятно [рассматривался](article|~/factorial|zero-factorial) в теме про факториалы.
...
@example
    ...
    sections:
        solution: |
            ...
            Найдём их количество с помощью [выведенной формулы](permutation-formula):
...
@example
    ...
    sections:
        solution: |
            Имеем $7$ цифр в распоряжении, **все** из которых придётся использовать, чтобы составлять семизначные числа.
            ...
            Найдём количество шестизначных чисел, которые можно составить из данных цифр.
...
@callout
    ...
    content: |
        ...
        --  [Юрий будет дуть](https://www.youtube.com/@vdud),
            Дуть будет Юрий,
            Будет Юрий дуть,
            Дуть Юрий будет,
            Юрий дуть будет,
...
Сделать это можно $P_2 = 2!$ способами: $м_1$, потом $м_2$ и наоборот, сначала $м_2$, потом $м_1$.
Точно так же есть $2!$ способов расставить элементы $а_1$ и $а_2$ на место букв "а".

Расстановка букв "м" никак не влияет на расстановку букв "а", поэтому, по [правилу произведения](~productRule), есть всего $2! \cdot 2! = 4$ способа составить слово "мама", в чём мы уже выше убедились.
...
В этом случае формула обычных перестановок перестаёт давать правильные ответы, ведь она повторяющиеся элементы считает уникальными.
Поэтому вместо неё надо использовать модифицированную формулу:

{ #permutation-repeat-formula +toc link="Перестановки с повторениями" }
@statement
    title: Формула перестановок с повторениями
    main: |
    ...
    sections:
        proof: |
            В этом доказательстве мы обобщим уже рассмотренный [пример](repetition-example).
            Обязательно изучите его и убедитесь, что во всём разобрались!
            ...
            Возьмём какую-нибудь уникальную перестановку (шаблон), например такую, в которой элементы из своих групп стоят рядом друг с другом.
            ...
            По такой же логике между собой можно менять местами элементы второй группы $n_2!$ способами, да хоть $k$-той группы $n_k!$ способами, и от этих перестановок сам шаблон останется точно таким же.
    ...
    sections:
        solution: |
            ...
            * Все остальные буквы в единственном экземпляре
            ...
            Найдём количество перестановок с повторениями по [выведенной формуле](permutation-repeat-formula):
...
@example
    ...
    sections:
        solution: |
            Ход решения точно такой же, как и в примере [Числовой мастер](number-master), только в этот раз у нас элементы (цифры) повторяются, поэтому нужно использовать [формулу](permutation-repeat-formula) перестановок с повторениями.
            Все уникальные перестановки $7$ цифр:

            $$ P(2, \ 1, \ 3, \ 1) = \frac{7!}{2! \ 1! \ 3! \ 1!} = \frac{5040}{12} = 420 $$

            Среди этих $420$ перестановок есть как искомые семизначные числа, так и шестизначные, начинающиеся с цифры $0$.
            Найдём все такие шестизначные числа, убрав из рассмотрения цифру $0$:

            $$ P(2, \ 3, \ 1) = \frac{6!}{2! \ 3! \ 1!} = 60 $$

            Осталось только вычесть из уникальных перестановок $7$ цифр все шестизначные числа, и мы найдём количество семизначных чисел:
...
{ #different-repeat +toc }
@important
    title: Повторения, но разные!
    main: |
        Не путайте "размещения с повторениями" и "перестановки с повторениями"!

        В размещениях с повторениями мы можем **неограниченное**{ accent } количество раз использовать один и тот же элемент.

        В перестановках с повторениями каждый элемент используется только **один раз**{ accent }, но среди самих элементов могут быть дубликаты в разных количествах!

# За рубежом

Если помните, в теме про размещения [говорилось](article|~/arrangement|foreign), что в англоязычной литературе само понятие "размещение" и обозначение $A$ не используют.
Размещения и перестановки там объединили в одно понятие, которое и назвали "перестановкой".

@important
    title: Языковой вопрос
    main: |
        У "нас" **две**{ accent } сущности: размещения ($A_n^k$) и перестановки ($P_n$), как частный случай размещений.

        У "них" **одна**{ accent } сущность -- перестановки, которые выполняют роль как размещений ($P_{n,k}$), так и перестановок ($P_n$) в нашем понимании.

{ link="Перестановки в жизни" }
# Анаграммы
...
Непросто приходилось учёным прошлого.
Интернета нет, научные журналы не выходили.
Заявить об открытии можно было либо в книгах, либо в частных письмах.
Написать и издать свою книгу -- это несколько лет непростой и недешёвой работы.
А писать письма -- ставить под удар своё открытие, которое может присвоить себе любой прочитавший письмо человек.

Защитить своё открытие можно, если шифровать письма.
С $17$ по $19$ века многие учёные краткой фразой формулировали суть открытия, переставляли в ней буквы и полученную анаграмму отсылали коллегам.
Получался своеобразный "анонс" гипотезы или открытия.
...
Ну а если гипотеза окажется неверной, её автор мог с таинственным видом сказать, что его анаграмма вообще про другое и избежать позора.
...
На самом деле, это анаграмма о его открытии.

Найдём количество возможных перестановок этих букв по [формуле](permutation-repeat-formula) перестановок с повторениями. Буква $S$ встречается $3$ раза, буква $M$ -- $5$ и так далее. Для сокращения записи не будем писать не влияющий на результат $1!$ для букв в единственном экземпляре:

$$ \frac{37!}{3! \ 5! \ 4! \ 4! \ 2! \ 2! \ 4! \ 3! \ 4! \ 2! } \approx 10^{33} $$

Согласитесь, неплохой способ защитить своё открытие.
Просто перемешал буквы и готово!
Расшифровывается анаграмма как "*altissimum planetam tergeminum observavi*" -- "*Высочайшую планету тройную наблюдал*", то есть речь идёт о Сатурне с двумя его "лунами" по бокам.
...
Он был убеждён, что его соперник обнаружил два спутника Марса. Спутники у Марса действительно есть, вот только открыли их лишь два столетия спустя.

## Открытие кольца Сатурна

В $1655$ году молодой астроном Христиан Гюйгенс нашёл разгадку загадочных "непостоянных лун" Сатурна, над которыми ломали голову Галилео Галилей и другие астрономы.
Он предположил, что это никакие не луны, а огромное кольцо вокруг газового гиганта.

Не будучи до конца уверенным в верности своей гипотезы, но и не желая терять первенство в потенциально громком открытии, в $1656$ он публикует "анонс" в виде анаграммы:

{ font-alt text-center }
*"aaaaaaa ccccc d eeeee g h iiiiiii llll mm nnnnnnnnn oooo pp q rr s ttttt uuuuu"*

Посчитаем количество вариантов составить текст из этих элементов по [формуле](permutation-repeat-formula) перестановок с повторениями:
...
Впрочем, зная контекст и используя правила языка, можно настолько упростить поиск расшифровки, что с этой задачей справится и человек, но всё равно будет очень непросто.
...
@todo
    ...
    content: |
        Было бы здорово дать ещё один или два красивых примера, как перестановки применяются в жизни, помимо анаграмм.