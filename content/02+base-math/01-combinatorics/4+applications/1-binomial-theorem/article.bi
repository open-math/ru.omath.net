...
Кто-то будет проводить умножение [напрямую](product-direct), кто-то прошаренный воспользуется [правилом фонтанчика](product-fountain).
В любом случае, немного повозившись, получаем ответ:

{ #product-direct title="Умножение многочленов напрямую" }
@details
...
{ #product-fountain title="Умножение многочленов фонтанчиком" }
@details
    Берём $a$ и по очереди умножаем его на каждое слагаемое второй скобки ($c, d, e$).
    Получаем слагаемые $ac, ad, ae$.
    Потом берём $b$ и делаем то же самое, получая слагаемые $bc, bd, be$.
...
@example
    ...
    sections:
        solution: |
            Три многочлена, значит, нам надо составить все возможные комбинации из $3$-х элементов, причём на каждое "вакантное место" в комбинации претендуют по $2$ буквы.
            ...
            Чтобы в процессе не запутаться, надо соблюдать [правила](article|~/brute-force|brute-force-rules) перебора.
...
@statement
    ...
    sections:
        "Примеры построения слагаемых": |
            ...
            Мы уже [знаем](polynom-product), как легко работать с произведением многочленов.
            ...
            Причём таких слагаемых будет аж $n$ штук, потому что в $a$ можно превратить любую из $n$ скобок (первую, вторую, $n$-ую):
        ...
        "Вывод формулы": |
            ...
            Порядок, в котором выбираются скобки, значения не имеет, поэтому каждый выбор скобок -- это [сочетание](article|~/combination|combination) из $n$ всех скобок по $k$ скобкам для превращения.
...
Возможно, кто-то из вас ожидал, что формула бинома Ньютона мы будем доказывать методом математической индукции.
Это [тоже можно сделать](practice|proof-by-induction), но объяснение через комбинаторные слагаемые выглядит более наглядно и интуитивно понятно.
Проблема любых доказательств по индукции в том, что они не дают понимания, как доказываемая формула вообще появилась, как к ней интуитивно пришли.
...
А если "членов" не два, а больше?
Для таких случаев есть расширение бинома Ньютона, которое называется [полиномиальной формулой](practice|polynomial-theorem)!

Бином Ньютона выглядит страшно замороченно, но стоит познакомиться с ним поближе, [попрактиковаться](page|practice|~/binomial-theorem) в его применении, и он станет незаменимым и очень полезным инструментом в арсенале любителя математики!
...
@example
    ...
    sections:
        solution: |
            ...
            Но вот для четвёртой степени умножать друг на друга $4$ скобки будет уже проблематично.
...
Выпишем только биномиальные коэффициенты из разложения $4$-ой степени бинома, которое получилось [после применения](short-product) формулы бинома Ньютона:
...
Причём коэффициенты после середины повторяют значения до середины, но в обратном порядке: первый биномиальный коэффициент равен первому с конца, второй равен второму с конца и так далее.
...
Заодно разберёмся, из-за чего это происходит:

{ #binom-coeff-repeat +toc link }
@statement
...
Полученные числа и есть искомые биномиальные коэффициенты.
...
@example
    ...
    sections:
        solution: |
            Ранее мы уже [построили](math:pascal-triangle) треугольник Паскаля для четвёртой степени бинома.
            Добавляем к нему ещё одну строчку.
...
Как пример "мостика" между суммами и точными формулами, с помощью бинома Ньютона можно [найти](page|article|/applications/sum-of-powers) общие формулы для решения очень красивой задачи о суммах степеней натуральных чисел (и не только чисел):
