...
@problem
    ...
    description: |
        ...
        $$ (a+b+c)(x+y+z)(n+m) $$
    hint: |
        Составьте все возможные комбинации из трёх элементов.
    ...
    solution: |
        Находим произведение трёх скобок "комбинаторным" способом, то есть выписываем все комбинации из трёх элементов. На первом месте в комбинации буквы из первой скобки, на втором -- из второй, на третьем из третьей.
...
@problem
    title: Сумма уникальных делителей
    level: easy
    attributes:
        - pretty
        - inter
    description: |
        Пускай $n$ -- натуральное число, все простые делители $p_1, p_2, \ldots$ которого **разные**:

        $$ n = p_1p_2p_3\ldots $$

        Докажите, что сумма $S$ всех его делителей равна произведению всех простых множителей числа $n$, увеличенных на единицу:

        $$ S_n = \underbrace{p_1 \cdot p_2 \cdot p_3 \cdot \ldots}_{\text{Простые множители}} \\ S = (p_1 + 1)(p_2 + 1)(p_3 + 1) \cdots $$

        Найдите сумму всех делителей числа $30$.
    hint: |
        Рассмотрите умножение скобок в формуле с точки зрения ["комбинаторного умножения" скобок](article|polynom-product).

        Любое умножение простых множителей числа $n$ дает его делитель.
    solution:
        Доказательство формулы: |
            В данной нам формуле суммы делителей при раскрытии всех скобок получится большая сумма слагаемых:

            $$ S = (p_1 + 1)(p_2 + 1)(p_3 + 1) \cdots = \\ p_1p_2p_3\ldots + 1p_2p_3\ldots + p_1 1 p_3\ldots + \ldots $$

            С точки зрения ["комбинаторного умножения" скобок](article|polynom-product) каждое слагаемое является уникальным набором произведения простых множителей числа:

            $$ p_1p_2p_3\ldots >> 111\ldots >> 1 p_2 1\ldots >> 11p_3\ldots $$

            Но любое произведение простых множителей числа $n$ является делителем этого числа $n$!
            Получается, раскрыв скобки мы и получаем искомую нами сумму всех возможных делителей числа $n$, начиная от самого $n = p_1p_2p_3\ldots$, включая все промежуточные, например $p_1 1 p_3 1 p_5 \ldots$ и заканчивая единицей $1 = 1\cdot 1\cdot 1\ldots$!

            $\blacksquare$
        Сумма делителей числа 30: |
            Теперь разберемся с числом $30$.
            Разложим его на множители:

            $$ 30 = 2 \cdot 3 \cdot 5 $$

            Все простые множители разные, значит можем использовать доказанную выше формулу суммы делителей:

            $$ S_{30} = (2 + 1)(3 + 1)(5 + 1) = 3 \cdot 4 \cdot 6 = 72 $$

            Сумма всех делителей числа $30$ равна $72$!
    answer: Сумма всех делителей числа $30$ равна $72$.
    note: |
        Приведенная в задаче формула суммы всех делителей числа $n$ работает только когда все простые множители этого числа разные.
        Если же они повторяются, то формула будет выглядеть иначе:

        $$ \sigma(n) = \left(\frac{p_1^{a_1 + 1} - 1}{p_1 - 1}\right) \cdot \left(\frac{p_2^{a_2 + 1} - 1}{p_2 - 1}\right) \cdots \left(\frac{p_k^{a_k + 1} - 1}{p_k - 1}\right) $$

        Эта формула тоже выводится довольно просто.
        Про нее, а также про формулы количества и произведение делителей вы можете почитать в теме ["Функции делителей"](page|article|/number-theory/divisor-functions) учебника по [теории чисел](page|book|/number-theory).
...
@problem
    ...
    description: |
        ...
        $$ (a+b)(a+c)(c+d)(n+m+e) $$
    hint: |
        Изучите решение [примера](article|polynom-product) из статьи.\
    ...
    solution: |
        ...
        С третьей то же самое, а четвёртую букву можно выбрать $3$ способами ($n, m$ или $e$).
...
@todo
    title: Задача на изучение подобия
    content: |
        Найдите количество подобных слагаемых после раскрытия скобок в выражении
...
@problem
    ...
    hint: |
        Воспользуйтесь [формулой](article|term) общего члена разложения или компактным видом [формулы](~binom) бинома Ньютона.
    solution:
        Через формулу общего члена: |
            Выпишем [формулу](article|term) общего члена разложения.

            $$ T_k = C_n^k a^{n-k}b^k $$

            Биномиальный коэффициент есть у каждого слагаемого.
            Начинается отсчёт с $0$ ($C_n^0$), а заканчивается на $n$ ($C_n^n$).
...
@problems
    ...
    level: easy
    shared: "Напишите формулу бинома Ньютона для степеней биномов:"
    set:
        -   description: |
                $$ (1+x)^7 $$
            hint: |
            ...
            solution: |
                Для начала проведём небольшое упрощение:

                @math
                    (1+x)^7 = \sum\limits_{k=0}^7 C_7^k 1^{7-k}x^k = \sum\limits_{k=0}^7 C_7^k x^k

                Теперь просто берём каждое значение $k$ от $0$ до $7$ и подставляем его в полученную упрощённую формулу:
...
@problems
    ...
    set:
        -   description: |
                ...
                $$ (x^2 + 4)^{10} $$
            hint: |
                Воспользуйтесь [формулой](article|term) общего члена разложения.
            solution: |
                Выпишем [формулу](article|term) общего члена разложения.
        ...
        -   description: |
                Найдите четвёртый член разложения степени бинома:
    ...
    set:
        -   description: |
            ...
            solution: |
                ...
                Подставим в неё данные из условия:
        ...
        -   description: |
            ...
            solution: |
                ...
                Нам нужно найти такой член разложения, в котором $x$ не присутствует.
        ...
        -   description: |
            ...
            solution: |
                ...
                Нам нужно найти такой член разложения, в котором $a$ не присутствует.
...
@problems
    ...
    set:
        -   description: |
                Найдите член разложения $(x + \sqrt{2})^{13}$, содержащий $x^8$
            hint: |
                Воспользуйтесь [формулой](article|term) общего члена разложения.
            solution: |
                Выпишем [формулу](article|term) общего члена разложения.
        ...
        -   description: |
                Найдите член разложения $(\sqrt{x} + 1)^{14}$, содержащий $x^6$
            solution: |
        ...
        -   description: |
                Найдите член разложения $(x^5 + 1)^{1980}$, содержащий $x^{1980}$
            solution: |
                Подставляем данные из условия в общий член разложения:
    ...
    set:
        -   description: |
            ...
            hint: |
                ...
                Ищите такие показатели степени, чтобы корни превращались в целые числа.
            solution: |
                ...
                Они всё равно никак не могут из корней сделать целые числа.
        ...
        -   description: |
                $$ (\sqrt{5} - \sqrt{2})^8 $$
            solution: |
                Имеем два множителя, причём каждый это корень второй степени, который превращается в целое число, когда его возводят в чётную степень: $0, 2, 4$ и так далее.

                Когда один множитель в чётной степени (например, во $2$), его сосед [тоже будет](even-explain) чётной степени ($8-2=6$).
                Поэтому в этом разложении в каждом чётном члене оба корня будут превращаться в целые числа, а всего таких чётных членов $5$.

                { #even-explain }
                @details
                    Чётные числа идут друг за другом как раз с шагом в $2$:
        ...
        -   description: |
            ...
            solution: |
                ...
                Его сосед $\sqrt{3}$ будет иметь показатель $124-4t$, что тоже является чётным числом, а значит и он тоже станет целым числом.
...
@problems
    ...
    set:
        -   description: |
                $(x + y)^{16}$ при $x^3y^{13}$
            hints:
                - "Воспользуйтесь [формулой](article|term) общего члена разложения бинома Ньютона."
                - В пунктах с трёхчленами выделите любые два одночлена скобками и рассматривайте полученные одночлены со скобками как биномы.
            solution: |
                Воспользуемся [формулой](article|term) общего члена разложения бинома Ньютона:
        ...
        -   description: |
                $(x + y + z)^{6}$ при $x^2y^2z^2$
            solution: |
                Выберем любые два одночлена, например $y$ и $z$, и поместим их в скобки.
                Тогда трёхчлен превращается в бином, который можно разложить по формуле бинома Ньютона, а значит можно использовать формулу общего члена разложения:
        ...
        -   description: |
            ...
            solution: |
                ...
                Впишем вместо скобок формулу общего её общего члена (единицу опускаем):
                ...
                Варианты $k=0$ и $k=2$ отбрасываем, потому что $2i$ даст чётное число и в сумме с уже имеющимися чётными показателями $0$ и $6$ число $7$ получить никак не выйдет.
        ...
        -   description: |
            ...
            solution: |
                ...
                Варианты $k=0$ и $k=2$ отбрасываем, потому что $2i$ даст чётное число и в сумме с уже имеющимися чётными показателями $0$ и $6$ число $11$ получить никак не выйдет.
...
@problems
    ...
    set:
        -   description: |
                $$ \left( \frac{1}{4} + \frac{3}{4}x \right)^4 $$
            hint: |
                Воспользуйтесь [формулой](article|term) общего члена разложения.
            solution: |
                Выпишем [формулу](article|term) общего члена разложения.
                ...
                $4^4$ это константа, её можно не рассматривать:
                ...
                Более маленькие значения $k$ можно не рассматривать, так как биномиальные коэффициенты будут [такими же](article|binom-coeff-repeat), а показатель степени тройки ещё меньше.
        ...
        -   description: |
            ...
            solution: |
                ...
                $3^{2024}$ это константа, её можно рассматривать:

                $$ C_{2024}^k \frac{2^k}{3^k} $$

                Найдём отношение "следующего" коэффициента к "предыдущему".
        ...
        -   description: |
            ...
            solution: |
                ...
                $x^k$ на коэффициент не влияет, $5^{10}$ это константа, её тоже можно не рассматривать.
                В итоге нам нужно максимизировать следующее выражение:

                $$ C_{20}^k (-1)^k \left(\frac{2}{5}\right)^{\small \frac{k}{2}} $$

                При нечётных $k$ коэффициент будет отрицательным, поэтому их сразу не рассматриваем, а заодно убираем $(-1)^k$:

                $$ C_{20}^k \left(\frac{2}{5}\right)^{\small \frac{k}{2}}, \quad k \ - \ чётное $$
                ...
                Максимальным по модулю коэффициент будет при $k = 7$, но так как $7$ число нечётное, то знак у коэффициента будет отрицательным.
...
@problem
    ...
    note: |
        ...
        Сочетание [является](article|~/combination|set-theory-example) множеством.
...
@problem
    ...
    hint: |
        Используйте результат задачи [Биномиальная сумма](coeff-sum).
    solution: |
        Пусть для освещения зала мы решили включить $k$ ламп.
        Выбрать $k$ ламп для включения из всех $10$ ламп [можно](lamp-variants) $C_{10}^k$ способами.

        { #lamp-variants }
        @details
            Порядок выбора $k$ ламп для включения не важен.
            При таком условии каждый способ включить лампы является [сочетанием](article|~/combination|combination) из $10$ доступных ламп по $k$ включенным лампам.
            [Количество](~combinations) таких сочетаний равно $C_{10}^k$.

        Но само количество включённых ламп $k$ может быть любым в промежутке от $1$ до $10$.
        ...
        Если мы добавим к этой сумме $C_{10}^0$ (все лампы выключены), то сможем воспользоваться полученным в задаче [Биномиальная сумма](coeff-sum) результатом:

        $$ C_{10}^0 + C_{10}^1 + C_{10}^2 + \ldots + C_{10}^{10} = 2^{10} $$

        Теперь вычтем из обеих частей добавленное лишнее слагаемое:
...
@problem
    ...
    hint: |
        ...
        Воспользуйтесь рекуррентной [формулой](article|~/factorial|factorial-recurrent) факториала.
    solution: |
        @math
            k C_n^k = n C_{n-1}^{k-1}
            \\
...
@problems
    ...
    level: hard
    shared: "Докажите равенства:"
    set:
        -   description: |
                $$ C_n^0 - C_n^1 + C_n^2 - \ldots + (-1)^{n} C_n^n = 0 $$
            hint: |
                Воспользуйтесь результатами задач [Биномиальная сумма](coeff-sum) и [Игра с коэффициентами](coeff-game).
            solution: |
                Сумма слева похожа на [формулу](~binom) бинома Ньютона, но как-бы "без" $a$ и $b$, только с биномиальными коэффициентами.
                Перепишем эту сумму так, чтобы получилось её "запаковать" при помощи бинома Ньютона при $a=1$ и $b=-1$:
        ...
        -   description: |
            ...
            solution: |
                ...
                Для каждого слагаемого применяем доказанное ранее [равенство](coeff-game):
                ...
                Теперь для суммы в скобках воспользуемся результатом задачи [Биномиальная сумма](coeff-sum):
        ...
        -   description: |
            ...
            solution: |
                Работаем с левой частью равенства и начинаем так же, как во второй подзадаче:

                $$ n \left( C_{n-1}^0 - C_{n-1}^1 + C_{n-1}^2 - \ldots + (-1)^{n-1} C_{n-1}^{n-1} \right) $$

                Сумма в скобках равна $0$ согласно первой подзадаче:
...
@problem
    ...
    solution:
        Доказательство через бином Ньютона: |
            В правой части равенства стоит биномиальный коэффициент из $2n$.
            ...
            Тут нам сильно пригодится умение ["комбинаторного" умножения](article|polynom-product), то есть возможности составлять комбинации из членов перемножаемых многочленов:
            ...
            Вспоминаем, что "противоположные" биномиальные коэффициенты [равны](article|binom-coeff-repeat):
        ...
        Комбинаторное доказательство: |
            Рассмотрим общий вид слагаемого в этой сумме и воспользуемся тем фактом, что "противоположные" биномиальные коэффициенты [равны](article|binom-coeff-repeat):
            ...
            Из первого набора мы берём $k$ элементов $C_n^k$ способами, а из второго $n-k$ элементов $C_n^{n-k}$ способами.
            ...
            * $\ldots$
            ...
            Но этот же результат можно получить напрямую, если свалить оба набора в одну кучу из $2n$ элементов и выбирать из неё сразу по $n$ элементов ($C_{2n}^n$).
...
@problem
    ...
    solution: |
        ...
        Разберёмся, чему равна целая часть после того, как мы применим к обеим степеням биномов [формулу](~binom) бинома Ньютона.
        ...
        Короче, все слагаемые с нечётной степенью взаимоуничтожаются и остаются только удвоенные чётные слагаемые:
        ...
        Во всех слагаемых внутри скобок $x$ в чётной степени, поэтому наибольшего значения вся эта скобка достигнет при $x=1$ (больше $1$ нельзя присвоить из-за ограничения в условии).
...
@problem
    ...
    hint: |
        Воспользуйтесь [правилом Паскаля](pascal-rule),  а также следующими очевидными равенствами:

        $$ C_n^0 = C_{n+1}^0 >> C_n^n = C_{n+1}^{n+1} $$
    ...
    solution:
        ...
        Индукционный переход: |
            ...
            Теперь сложем всё это дело друг с другом, вынося за скобки $a$ и $b$ в одинаковых степенях:

            $$ C_t^0 a^{t+1} + a^tb(C_t^1 + C_t^0) + a^{t-1}b^2(C_t^2 + C_t^1) + \ldots + ab^t(C_t^t + C_t^{t-1}) + C_t^t b^{t+1} $$

            Для двух крайних слагаемых производим [замену](induction-swap-explain) $C_t^0 = C_{t+1}^0$ и $C_t^t = C_{t+1}^{t+1}$.
            Каждую скобку с суммой биномиальных коэффициентов заменяем по [правилу Паскаля](pascal-rule).
            ...
            @details
                ...
                $C_t^t$ [означает](article|~/combination|combination-purpose) количество способов выбрать $t$ предметов, когда всего их $t$.
            ...
            Всё это время мы работали с правой частью [исходного равенства](induction-base-equality):
...
@problem
    ...
    level: hard
    description: |
        Пусть $M_r$ -- число [размещений без повторений](article|~/arrangement|a-unique) из $m$ элементов по $r$, а $N_r$ -- число размещений без повторений из $n$ элементов по $r$.
        Докажите, что число размещений из $m+n$ элементов по $r$ выражается формулой $(M+N)^r$, где в разложении надо заменить все показатели степени индексами.
    hint: |
...
@problem
    ...
    hint: |
        Используйте понятие перестановок с повторениями и [формулу](article|~/permutation|permutation-repeat-formula) их количества.
    solution: |
        ...
        Столько, сколькими способами можно $n$ разложить на сумму из $m$ слагаемых по показателем степеней одночленов исходного полинома ($x_1, \ x_2$ и так далее).
        ...
        Чтобы посчитать количество подобных нам понадобится понимание ["комбинаторного" умножения](article|polynom-product) многочленов.
        ...
        Тогда количество подобных будет равно [количеству](article|~/permutation|permutation-repeat-formula) перестановок с повторениями:
...
@problem
    ...
    description: |
        ...
        $$ (x_1 + x_2 + \ldots + x_m)^n $$
    hint: |
        Изучите [вывод](polynomial-theorem) полиномиальной формулы.\
        Вопрос о количестве способов разбить число на слагаемые уже [рассматривался](practice|~/combination|number-composition-zeros) раньше.
    solution: |
        Из [вывода](polynomial-theorem) полиномиальной формулы мы знаем, что количество слагаемых в разложении степени полинома равно количеству способов эту самую степень полинома $n$ разбить на $m$ слагаемых -- показателей степеней одночленов исходного полинома ($x_1, \ x_2$ и так далее).
        Причём показатель степени, а значит и слагаемое может равняться $0$.

        А количество способов разбить число $n$ на $m$ слагаемых (включая $0$) мы [выводили](practice|~/combination|number-composition-zeros) в практикуме на сочетания.
...
@problem
    ...
    description: |
        ...
        $$ a + b + c + d $$
    hint: |
        Воспользуйтесь полиномиальной [формулой](polynomial-theorem).
    solution: |
